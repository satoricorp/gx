...
import txReview, { metadata as reviewMetadata, schema as reviewSchema } from "../src/tools/tx-review";

describe("tx_review metadata and schema", () => {
  test("describes the review surface", () => {
    expect(reviewMetadata.name).toBe("tx_review");
...
describe("registered MCP tool names", () => {
  test("exposes exactly tx_review", () => {
    const registered = [reviewMetadata.name].sort();
    expect(registered).toEqual(["tx_review"]);
    expect(registered).not.toContain("tx_commit");
    expect(registered).not.toContain("tx_status");
    expect(registered).not.toContain("tx_push");
    expect(registered).not.toContain("tx_publish");
    expect(registered).not.toContain("tx_edit");
  });
});

describe("tx MCP CLI invocation", () => {
  let mockDir: string;
  let repoRoot: string;
  let callLog: string;
  ...
  beforeEach(async () => {
    mockDir = await mkdtemp(join(tmpdir(), "tx-mcp-tools-"));
    ...
    const mockTt = join(mockDir, "mock-tx.sh");
    await writeFile(
      mockTt,
      `#!/bin/sh
...
if [ "$1" = review ]; then
  if [ "$TOTALITY_MOCK_AUTH_ERROR" = "1" ]; then
    echo 'github token is not configured for MCP: run \`tx auth login\` in a terminal, then retry the MCP tool' >&2
    exit 1
  fi
  echo "review ok"
  exit 0
...
if [ "$1" = doctor ]; then
  if [ ! -f "$PWD/.totality-initialized" ]; then
    echo "tx not initialized" >&2
    exit 1
  fi
  echo '{"doctor":{}}'
  exit 0
fi
echo "unexpected tx args: $@" >&2
exit 1
`,
      "utf8",
    );
  ...
  test("tx_review passes scope, focus, prompt, deep, and verbose flags", async () => {
    const output = await txReview({
      cwd: repoRoot,
      scope: "architecture",
  ...
  test("tx_review can ask for a whole-repo review", async () => {
    ...
    const output = await txReview({ cwd: repoRoot, repo: true });
    const parsed = JSON.parse(output);
    ...
    const plain = JSON.parse(await txReview({ cwd: repoRoot }));
    expect(plain.command).toEqual([process.env.TOTALITY_BINARY, "review", "--no-publish"]);
  });

  test("tx_review never publishes, whatever it is asked for", async () => {
    // readOnlyHint is a promise to the calling agent. Without --no-publish,
    // `tx review` posts a comment on the matching GitHub PR and records the
    ...
    for (const variant of variants) {
      const parsed = JSON.parse(await txReview({ cwd: repoRoot, ...variant }));
      expect(parsed.command).toContain("--no-publish");
    }
  });

  test("tx_review never initializes the repo it reviews", async () => {
    ...
    const output = await txReview({ cwd: repoRoot });
  ...
  test("tx_review surfaces MCP auth login guidance", async () => {
    ...
    expect(parsed.display).toBe(
      "Totality cloud authentication is required. Run `tx auth login` in a terminal, then retry the MCP tool.",
    );
    expect(parsed.stderr).toContain("run `tx auth login` in a terminal");
    expect(parsed.next_actions[0]).toBe("Run `tx auth login` in a terminal, then retry the MCP tool.");
  });
});
