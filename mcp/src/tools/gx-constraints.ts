...
export const schema = {
  ...
  prompt: z
    ...
    .optional()
    .describe(
      "Optional intent hint passed as the positional gx constraints argument: what the change is meant to do, so the back-pressure gate can judge scope against it.",
    ),
...
export default async function gxConstraints(params: InferSchema<typeof schema>) {
  // --report-only is not optional here. `gx constraints` exits non-zero on a
  ...
  const args = ["constraints", "--report-only", "--md", "--client", "mcp"];
  if (params.verbose) {
    args.push("--verbose");
  }
  ...
  try {
    ...
    return formatError(error, { action: "constraints" });
  }
}
