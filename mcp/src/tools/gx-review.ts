...
export const schema = {
  ...
  prompt: z
    ...
    .optional()
    .describe(
      "Optional intent hint passed as the positional gx review argument: what the change is meant to do, so the back-pressure gate can judge scope against it.",
    ),
...
export default async function gxReview(params: InferSchema<typeof schema>) {
  // --report-only is not optional here. `gx review` exits non-zero on a
  ...
  const args = ["review", "--report-only", "--md", "--client", "mcp"];
  if (params.verbose) {
    args.push("--verbose");
  }
