...
4. `gx_review` before shipping: the pre-ship exit gate that checks the change against six gates and returns a ship/no-ship verdict.
...
Before shipping, run `gx_review` (MCP) or `gx review` (CLI) to check the change against the pre-ship gates.
...
| `gx_review` | `gx review --report-only --md --client mcp [hint]` | Pre-ship exit gate: six gate checks with a ship/no-ship verdict |
...
`gx_review` always passes `--report-only` and `--md`. The CLI's coded
...
and `gx_review` MCP tools are read-only — they never initialize a repo,
