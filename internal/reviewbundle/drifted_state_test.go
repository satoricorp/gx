...
func TestBuildPushReadsTheDriftedRowThePushWroteTo(t *testing.T) {
	...
	// The stale row: what an older lgtm wrote before the identity key moved to
	// the git common dir. Live analogue: repos id 1, 17 changes, last touched
	// six days before the row that now receives every write.
	staleChange := h.SeedChange(t, h.LegacyRepoID, "lgtmr-drift", "commit-drift", "stale alpha", []string{"stale.go"})
	...
	currentChange := h.SeedChange(t, h.CurrentRepoID, "lgtmr-drift", "commit-drift", "real alpha", []string{"real.go"})
...
//
// lgtm's own demux-worktree flow and the yeet harness both push from linked
...
func TestBuildPushFromLinkedWorktreeOnADriftedDatabase(t *testing.T) {
	...
	changeID := h.SeedChange(t, h.CurrentRepoID, "lgtmr-worktree", "commit-worktree", "gamma", []string{"gamma.go"})
