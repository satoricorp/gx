...
func TestLegacyLaunchAgentPathUsesHomeLaunchAgents(t *testing.T) {
	...
	want := filepath.Join(home, "Library", "LaunchAgents", "dev.lgtm.capture.plist")
	if got != want {
...
func TestLegacyLaunchAgentTargetUsesGUIDomain(t *testing.T) {
	want := "gui/" + strconv.Itoa(os.Getuid()) + "/dev.lgtm.capture"
...
func TestLegacyProxyBaseURLs(t *testing.T) {
	...
	t.Setenv("LGTM_PROXY_ADDR", "127.0.0.1:50000")
	anthropicURL, openaiURL = legacyProxyBaseURLs()
	if anthropicURL != "http://127.0.0.1:50000" || openaiURL != "http://127.0.0.1:50000/v1" {
		t.Fatalf("legacyProxyBaseURLs() with LGTM_PROXY_ADDR = %q, %q", anthropicURL, openaiURL)
	}
}
...
func writeLegacyPlist(t *testing.T, home string) string {
	...
	path := filepath.Join(dir, "dev.lgtm.capture.plist")
...
func TestCleanupLegacyAmbientCaptureRemovesArtifacts(t *testing.T) {
	...
	if strings.Contains(string(data), "lgtm ambient capture") {
...
func legacyCodexBackups(t *testing.T, path string) []string {
	t.Helper()
	matches, err := filepath.Glob(path + ".lgtm-backup-*")
	if err != nil {
		t.Fatal(err)
	}
	return matches
...
func TestRevertLegacyCodexConfigFileFullRevert(t *testing.T) {
	...
	fixture := strings.Join([]string{
		`# personal settings`,
		`model = "gpt-5"`,
		`model_provider = "lgtm-openai"`,
		``,
		`[projects."/repo"]`,
		`trust_level = "trusted"`,
		``,
		`[model_providers.lgtm-openai]`,
		`name = "lgtm OpenAI Proxy"`,
...
func TestRevertLegacyCodexConfigFileHonorsLgtmProxyAddr(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("LGTM_PROXY_ADDR", "127.0.0.1:50000")
	...
	if strings.Contains(string(data), "lgtm-openai") {
		t.Fatalf("reverted config still mentions lgtm-openai:\n%s", data)
	}
	...
	if !legacyCodexBaseURLPointsAtProxy("http://127.0.0.1:43123/v1") {
		t.Fatal("default proxy address no longer recognized with LGTM_PROXY_ADDR set")
	}
}
...
func TestLegacyCodexConfigPointsAtRetiredProxy(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			text: "[model_providers.lgtm-openai]\nbase_url = \"http://127.0.0.1:43123/v1\"\n",
			want: true,
		},
		{
		...
		},
		{
			...
			text: "model_provider = \"lgtm-openai\"\n",
			want: true,
		},
		{
...
func TestCleanupLegacyAmbientCaptureRevertsCodexConfig(t *testing.T) {
	...
	if strings.Contains(string(data), "lgtm-openai") {
		t.Fatalf("codex config still mentions lgtm-openai:\n%s", data)
	}
}
