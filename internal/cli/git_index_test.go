...
func TestReadOnlyCommandsPreserveGitIndex(t *testing.T) {
	...
	if len(before) == 0 {
		t.Fatal("expected staged content before running lgtm")
	}

	lgtmHome := t.TempDir()
	writeTestLgtmConfig(t, lgtmHome, "Test", "t@e.com")
	...
	for _, mode := range modes {
		t.Run(mode.name, func(t *testing.T) {
			...
			if err := cmd.Execute(); err != nil {
				...
				t.Fatalf("lgtm %s error = %v", strings.Join(mode.args, " "), err)
			}
			after := gitCachedDiff(t, root)
			if string(after) != string(before) {
				t.Fatalf("git index changed after lgtm %s\nbefore=%q\nafter=%q", strings.Join(mode.args, " "), before, after)
			}
		})
	}
}
