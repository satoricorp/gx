...
// runGlobalInit installs the machine-wide lgtm lifecycle hooks. It does not need
...
func runGlobalInit(ctx context.Context, cmd *cobra.Command, quiet bool) error {
	...
	if result.ConfigUpdated {
		...
		fmt.Fprintln(out, labelValue("Git config", muted("core.hooksPath already pointed at lgtm")))
	}
	...
	fmt.Fprintln(out, labelValue("Opt out", "git config lgtm.enabled false          (one repo)"))
