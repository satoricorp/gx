...
// isolateGlobalGitConfigForCLI points HOME, LGTM_HOME and git's global/system
// config at throwaway files so `lgtm init --global` can be executed in tests
// without ever touching the developer's real ~/.gitconfig or ~/.lgtm.
...
func TestInitGlobalInstallsMachineWideHooks(t *testing.T) {
	...
	if err := root.Execute(); err != nil {
		t.Fatalf("lgtm init --global error = %v\n%s", err, out.String())
	}

	hooksDir := filepath.Join(os.Getenv("LGTM_HOME"), "hooks")
	for _, name := range []string{"prepare-commit-msg", "post-commit", "post-rewrite", "pre-push", "pre-commit"} {
		...
		if !strings.Contains(string(data), "lgtm_resolve_local_hook "+name) {
	...
	for _, want := range []string{
		"Global hooks",
		hooksDir,
		"git config lgtm.enabled false",
		"git config --global --unset core.hooksPath",
	} {
		if !strings.Contains(text, want) {
			t.Fatalf("lgtm init --global output missing %q:\n%s", want, text)
		}
	}
}
...
func TestInitGlobalRefusesForeignHooksPath(t *testing.T) {
	...
	err := root.Execute()
	if err == nil {
		t.Fatalf("lgtm init --global error = nil, want refusal\n%s", out.String())
	}
	if !strings.Contains(err.Error(), "not managed by lgtm") || !strings.Contains(err.Error(), foreign) {
...
func TestInitGlobalFlagIsOptIn(t *testing.T) {
	...
	if flag == nil {
		t.Fatal("lgtm init has no --global flag")
	}
	...
	for _, name := range []string{"name", "email", "yes"} {
		if init.Flags().Lookup(name) == nil {
			t.Fatalf("lgtm init lost its --%s flag", name)
		}
	}
}
...
func TestCaptureGlobalHooksDoctorValue(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "not opted in", state: globalHookDoctorJSON{Dir: "/home/dev/.lgtm/hooks"}, want: ""},
		{
			name:  "enabled",
			state: globalHookDoctorJSON{Dir: "/home/dev/.lgtm/hooks", Enabled: true, Installed: true},
			want:  "ok: /home/dev/.lgtm/hooks",
		},
		{
			name:  "overridden",
			state: globalHookDoctorJSON{Dir: "/home/dev/.lgtm/hooks", Installed: true, Conflict: "/home/dev/.husky", NeedsRepair: true},
			want:  "warn: core.hooksPath points at /home/dev/.husky; run `lgtm init --global` to restore",
		},
		{
			name:  "incomplete",
			state: globalHookDoctorJSON{Dir: "/home/dev/.lgtm/hooks", Enabled: true, NeedsRepair: true},
			want:  "warn: incomplete; run `lgtm init --global`",
		},
	} {
