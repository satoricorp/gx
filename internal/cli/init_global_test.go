...
// config at throwaway files so `tl init --global` can be executed in tests
...
func TestInitGlobalInstallsMachineWideHooks(t *testing.T) {
	...
	if err := root.Execute(); err != nil {
		t.Fatalf("tl init --global error = %v\n%s", err, out.String())
	}
	...
	for _, name := range []string{"prepare-commit-msg", "post-commit", "post-rewrite", "pre-push", "pre-commit"} {
		...
		if !strings.Contains(string(data), "tl_resolve_local_hook "+name) {
	...
	for _, want := range []string{
		...
		if !strings.Contains(text, want) {
			t.Fatalf("tl init --global output missing %q:\n%s", want, text)
		}
	}
}
...
func TestInitGlobalRefusesForeignHooksPath(t *testing.T) {
	...
	err := root.Execute()
	if err == nil {
		t.Fatalf("tl init --global error = nil, want refusal\n%s", out.String())
	}
...
func TestInitGlobalFlagIsOptIn(t *testing.T) {
	...
	if flag == nil {
		t.Fatal("tl init has no --global flag")
	}
	...
	for _, name := range []string{"name", "email", "yes"} {
		if init.Flags().Lookup(name) == nil {
			t.Fatalf("tl init lost its --%s flag", name)
		}
	}
}
...
func TestCaptureGlobalHooksDoctorValue(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			want:  "warn: core.hooksPath points at /home/dev/.husky; run `tl init --global` to restore",
		},
		{
			name:  "incomplete",
			state: globalHookDoctorJSON{Dir: "/home/dev/.totality/hooks", Enabled: true, NeedsRepair: true},
			want:  "warn: incomplete; run `tl init --global`",
		},
	} {
