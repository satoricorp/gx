...
var reportRedactors = []*regexp.Regexp{
	...
	// CLI session tokens. Deliberately matches every prefix this product has
	// ever minted, not just the current one: a report is uploaded, a stale
	// config on disk holds a real token, and a redactor that recognizes fewer
	// shapes than exist is the one failure mode here that cannot be undone.
	regexp.MustCompile(`(?:gx|gx|tl)cs_[A-Za-z0-9._-]+`),
}
