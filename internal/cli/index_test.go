...
func TestIndexCommandIsRegisteredAsHidden(t *testing.T) {
	...
	if !cmd.Hidden {
		t.Fatal("lgtm index is a maintenance command and must stay hidden")
	}
	if cmd.GroupID != "" {
		t.Fatalf("lgtm index GroupID = %q, want no group", cmd.GroupID)
	}
	for _, flag := range []string{"full", "json", "quiet", "concurrency", "namespace"} {
		if cmd.Flags().Lookup(flag) == nil {
			t.Fatalf("lgtm index is missing the --%s flag", flag)
		}
	}
}
...
func TestIndexCommandReportsMissingCredentials(t *testing.T) {
	...
	err := root.Execute()
	if err == nil {
		t.Fatal("lgtm index without credentials must return an error, not silently do nothing")
	}
