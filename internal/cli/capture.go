...
import (
	...
	"github.com/satoricorp/lgtm/internal/capture/reparse"
...
func newCaptureSyncCommand(ctx context.Context) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "sync",
		Short: "Upload shareable capture staging rows to the lgtm server",
...
func runCaptureSync(ctx context.Context, out interface{ Write([]byte) (int, error) }, quiet bool) error {
	...
	if !ok {
		...
		return fmt.Errorf("not logged in for capture upload — run `lgtm auth login`")
	}
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if outcome.SkipReason != "" {
				...
				if result.AttributionSuspect {
					fmt.Fprintln(cmd.ErrOrStderr(), labelWarningValue("Warning", fmt.Sprintf(
						"found agent sessions but linked none of %d changed hunk(s); session data was likely still being written. Re-run once the session settles: lgtm capture push --repo %s --ref-range %s",
			...
			if outcome.RecoveryError != "" {
				fmt.Fprintf(cmd.ErrOrStderr(), "warning: repair lgtm revision metadata: %s\n", outcome.RecoveryError)
			}
			...
			if outcome.PublicationError != "" {
				fmt.Fprintf(cmd.ErrOrStderr(), "warning: this push queued no lgtm review artifact: %s\n", outcome.PublicationError)
			}
...
func installCaptureHookWithOutput(cmd *cobra.Command, repoRoot string, printSuccess bool) error {
	...
	if err := hooks.Install(hooks.InstallOptions{RepoRoot: repoRoot, LgtmPath: lgtmPath}); err != nil {
		return err
	}
	if printSuccess {
		fmt.Fprintln(cmd.OutOrStdout(), labelValue("lgtm lifecycle hooks", success("ok")+": commit metadata and push capture"))
	}
	return nil
}
...
func installClaudeCaptureHooks(cmd *cobra.Command, repoRoot string, quiet bool) error {
	...
	if err := claudehooks.MergeSettings(repoRoot, lgtmPath); err != nil {
		return err
	}
	if !quiet {
