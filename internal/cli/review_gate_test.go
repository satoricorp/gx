...
func commitReviewSecret(t *testing.T, root string) {
	t.Helper()
...
func TestReviewExitsThreeOnNoShip(t *testing.T) {
	root := newReviewGateRepo(t)
	commitReviewSecret(t, root)
	t.Chdir(root)
	setReviewGateEnv(t)

	out, err := runReviewCommand(t)
	if err == nil {
...
func TestReviewExitsZeroOnShip(t *testing.T) {
	root := newReviewGateRepo(t)
...
func TestReviewExitsFourOnNothingToCheck(t *testing.T) {
	root := newReviewGateRepo(t)
	t.Chdir(root)
	setReviewGateEnv(t)

	_, err := runReviewCommand(t)
...
func TestReviewReportOnlyExitsZero(t *testing.T) {
	root := newReviewGateRepo(t)
	commitReviewSecret(t, root)
	t.Chdir(root)
	setReviewGateEnv(t)

	out, err := runReviewCommand(t, "--report-only")
	if err != nil {
...
func TestReviewJSONRoundTrips(t *testing.T) {
	root := newReviewGateRepo(t)
	commitReviewSecret(t, root)
	t.Chdir(root)
	setReviewGateEnv(t)

	out, err := runReviewCommand(t, "--json", "--report-only")
	if err != nil {
		t.Fatalf("gx review --json error = %v\n%s", err, out)
	}
	var report codereview.ReviewReport
...
func TestReviewMarkdownEmitsTheTable(t *testing.T) {
	root := newReviewGateRepo(t)
	commitReviewSecret(t, root)
	t.Chdir(root)
	setReviewGateEnv(t)

	out, err := runReviewCommand(t, "--md", "--report-only")
	if err != nil {
		t.Fatalf("gx review --md error = %v\n%s", err, out)
	}
...
func TestReviewRejectsUnknownSkipGate(t *testing.T) {
	root := newReviewGateRepo(t)
	commitOnBranch(t, root)
	t.Chdir(root)
	setReviewGateEnv(t)

	_, err := runReviewCommand(t, "--skip-gates", "correctness,typo")
