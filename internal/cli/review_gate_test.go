...
func setReviewGateEnv(t *testing.T) {
	...
	t.Setenv("LGTM_REVIEW_HISTORY_CONTEXT", "0")
	denyReviewNetwork(t)
}

// denyReviewNetwork makes a review test hermetic.
//
// These tests inherit the developer's shell, and a developer working on lgtm has
...
// created a brand new `lgtm-local-*` namespace that nothing would ever delete.
//
// Clearing the credentials is the mechanism rather than a per-feature kill
// switch such as LGTM_REVIEW_CODE_INDEX=0, because it shuts every door at once:
...
func denyReviewNetwork(t *testing.T) {
	...
	t.Setenv("LGTM_OPENAI_BASE_URL", server.URL)
	t.Setenv("LGTM_TPUF_BASE_URL", server.URL)
}
...
func TestReviewJSONReportsTheResolvedCommitRange(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("lgtm review --json error = %v\n%s", err, out)
	}
...
func TestReviewJSONMarksNothingToReview(t *testing.T) {
	...
	if strings.TrimSpace(report.ReviewTarget) == "" {
		t.Fatalf("review_target is empty, want the refs lgtm looked at:\n%s", out)
	}
}
...
func TestReviewNothingToReviewDoesNotClaimACleanReview(t *testing.T) {
	...
	if strings.Contains(out, "No material issues found in this change.") {
		t.Fatalf("lgtm review claimed a clean review without inspecting anything:\n%s", out)
	}
	if !strings.Contains(out, "Nothing to review") {
		t.Fatalf("lgtm review output missing the nothing-to-review outcome:\n%s", out)
	}
}
...
func TestReviewFailOnExitsDistinctlyWhenNothingWasReviewed(t *testing.T) {
	...
	if _, err := runReviewCommand(t, "--no-publish"); err != nil {
		t.Fatalf("lgtm review without --fail-on error = %v", err)
	}
}
...
func TestReviewOnAnEmptyRepoNeverPassesAGate(t *testing.T) {
	for _, flags := range [][]string{nil, {"--repo"}} {
		...
		t.Run(name, func(t *testing.T) {
			...
			if gateErr == nil {
				t.Fatalf("lgtm review --fail-on any passed on an empty repo:\n%s", gateOut)
			}
...
// The lgtm Cloud history row and the PostHog event both label the run, and they
...
func TestReviewBaseFlagSelectsTheRange(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("lgtm review --base error = %v\n%s", err, out)
	}
	var report codereview.Report
	if decodeErr := json.Unmarshal([]byte(out), &report); decodeErr != nil {
		t.Fatalf("json.Unmarshal() error = %v, output:\n%s", decodeErr, out)
...
func TestReviewRepoFlagReviewsTheRepositoryWithADirtyTree(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("lgtm review --repo error = %v\n%s", err, out)
	}
	var report codereview.Report
	if decodeErr := json.Unmarshal([]byte(out), &report); decodeErr != nil {
		t.Fatalf("json.Unmarshal() error = %v, output:\n%s", decodeErr, out)
...
func TestReviewRepoFlagReviewsTheRepositoryWithACleanTree(t *testing.T) {
	...
	if strings.Contains(out, "Nothing to review") {
		t.Fatalf("lgtm review --repo reported nothing to review:\n%s", out)
	}
	if !strings.Contains(out, "Reviewed the repository") {
		t.Fatalf("lgtm review --repo does not say what it reviewed:\n%s", out)
	}
	...
	if err != nil {
		t.Fatalf("lgtm review --repo --json error = %v\n%s", err, jsonOut)
	}
	var report codereview.Report
	...
	if !strings.Contains(plain, "Nothing to review") {
		t.Fatalf("lgtm review lost the nothing-to-review outcome:\n%s", plain)
	}
}
...
func TestReviewRepoFlagWinsOverBaseAndSaysSo(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("lgtm review --repo --base error = %v\n%s", err, out)
	}
	var report codereview.Report
	if decodeErr := json.Unmarshal([]byte(out), &report); decodeErr != nil {
		t.Fatalf("json.Unmarshal() error = %v, output:\n%s", decodeErr, out)
...
func TestReviewRejectsUnknownFailOnLevel(t *testing.T) {
	...
	if err == nil {
		t.Fatal("lgtm review accepted an unknown --fail-on level")
	}
