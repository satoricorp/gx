...
func TestRenderMarkdownStatesUnavailableEvidence(t *testing.T) {
	report := Report{
		...
		Evidence: []EvidenceStatus{
			{Source: "code index", Namespace: "repo-satoricorp-yeet", State: EvidenceMissing, Detail: "lgtm Cloud has never indexed this repository"},
	...
	if !strings.Contains(markdown, "lgtm Cloud has never indexed this repository") {
	...
	// meaningful.
	healthy := report
	healthy.Evidence = []EvidenceStatus{{Source: "code index", Namespace: "repo-satoricorp-lgtm", State: EvidenceOK, Snippets: 12}}
...
func TestRenderMarkdownListsEveryEvidenceSourceWhenVerbose(t *testing.T) {
	report := Report{
		...
		Verbose:    true,
		Evidence: []EvidenceStatus{
			{Source: "code index", Namespace: "repo-satoricorp-lgtm", State: EvidenceOK, Snippets: 12},
			{Source: "indexed sessions", State: EvidenceDisabled, Detail: "TURBOPUFFER_API_KEY is not set"},
		},
	}
	markdown := RenderMarkdown(report)
	for _, want := range []string{"Evidence: code index (repo-satoricorp-lgtm): ok, 12 snippet(s)", "Evidence: indexed sessions: disabled"} {
...
func TestMissingNamespacesAreExpectedWhenAnotherAnswered(t *testing.T) {
	if warnings := EvidenceWarnings([]EvidenceStatus{
		{Source: "code index", Namespace: "lgtm-local-yeet-8d86-v2", State: EvidenceOK, Snippets: 86},
		{Source: "code index", Namespace: "repo-satoricorp-yeet", State: EvidenceMissing, Detail: "lgtm Cloud has never indexed this repository"},
	}); len(warnings) != 0 {
		t.Fatalf("warnings = %v, want none: the code index answered", warnings)
	}
	if warnings := EvidenceWarnings([]EvidenceStatus{
		{Source: "code index", Namespace: "lgtm-local-yeet-8d86-v2", State: EvidenceEmpty},
