...
func TestGatesFindingFromAICarriesRuleID(t *testing.T) {
	known := KnownRuleIDs(AllRuleDefs(nil)...)
	f := gatesFindingFromAI(GateBackPressure, gatesAIFinding{
		Title: "t", Summary: "s", Recommendation: "r", RuleID: "api-contract-misuse",
	}, known)
	if f.RuleID != "review/api-contract-misuse" {
		t.Errorf("gates finding rule id = %q", f.RuleID)
	}
	if f.ID != "gates.back-pressure" {
		t.Errorf("gates positional ID disturbed: %q", f.ID)
	}
	...
	packed := gatesFindingFromAI(GateCodeHealth, gatesAIFinding{Title: "t", RuleID: "gx:recommended/no-swallowed-errors"}, known)
	...
	unregistered := gatesFindingFromAI(GateCodeHealth, gatesAIFinding{Title: "t", RuleID: "REVIEW.md/not-declared"}, known)
