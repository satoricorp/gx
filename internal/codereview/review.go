...
type Report struct {
	...
	// AWS credentials and run through lgtm Cloud has different latency, different
	...
	// ReviewTarget is the human phrase naming what was inspected, or where lgtm
...
func RenderMarkdown(report Report) string {
	...
	if report.ReviewMode == ReviewModeRepo {
		...
		// the lgtm Cloud history entry this same text becomes. Both paths are
...
func skipDir(rel string) bool {
	for _, part := range strings.Split(rel, "/") {
		switch part {
		case ".git", ".jj", ".lgtm", ".gocache", "node_modules", "dist", "build", ".next", "coverage", ".cache", ".turbo", "vendor":
			return true
		}
	}
	return false
...
func changedFiles(ctx context.Context, repoRoot string) []string {
	...
	// a single `internal/lgtmtest/` entry. That entry is not a file: it produces
