...
func staticToolFailureFindings(ctx ReviewContext) []Finding {
	...
	return []Finding{{
		...
		Evidence:       evidence,
		Recommendation: "Start by fixing the failing tool output, then rerun `lgtm review` so the reviewer can judge structure on a clean baseline.",
		Strength:       "Blocking",
		SourceIDs:      []string{"google-eng-practices"},
	}}
}
...
func missingAgentsFinding(facts RepoFacts) Finding {
	...
	return Finding{
		...
		Benefit: "Reduces failed agent runs and accidental workflow drift by making build, test, and lgtm/JJ conventions explicit.",
		Evidence: []Evidence{
...
func noDependencyManifestFinding(facts RepoFacts) Finding {
	...
	return Finding{
		...
		Benefit: "Improves dependency review and reproducibility by making the build inputs visible to lgtm, CI, and teammates.",
		Evidence: []Evidence{
