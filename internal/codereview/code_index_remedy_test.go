...
//
// Every route goes to the website. `lgtm index` is a hidden maintenance command
// that fills one developer's namespace from one developer's checkout; the
// index a review actually reads is the one lgtm Cloud maintains from the GitHub
...
func TestMissingIndexWarningsCarryTheirRemedy(t *testing.T) {
	...
	statuses := []EvidenceStatus{
		{
			Source: codeIndexEvidenceSource, Namespace: "lgtm-001-acme-api-v2", State: EvidenceMissing,
		...
		{
			Source: codeIndexEvidenceSource, Namespace: "lgtm-local-api-abc-v2", State: EvidenceMissing,
	...
	warning := warnings[0]
	if !strings.Contains(warning, "https://lgtm.cx/repositories") {
		t.Fatalf("warning = %q, want it to say where to get the repository indexed", warning)
	}
	if strings.Contains(warning, "lgtm index") {
		t.Fatalf("warning = %q, want the website rather than the hidden `lgtm index` command", warning)
	}
	if count := strings.Count(warning, "https://lgtm.cx/repositories"); count != 1 {
...
func TestEveryMissingCodeIndexOriginHasARemedy(t *testing.T) {
	for _, origin := range []string{
		...
		if remedy := codeIndexMissingRemedy(target); !strings.Contains(remedy, "https://lgtm.cx/repositories") {
