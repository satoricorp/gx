...
// answer to "what did lgtm read?", and the difference between "reviewed and
...
// resolved, names that diff too, so the report never implies lgtm ignored it.
...
// noCommittedChangeTarget names everywhere lgtm looked, so a "nothing to review"
