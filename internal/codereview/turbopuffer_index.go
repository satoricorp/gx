...
// index is 1536-dimensional with a `content` column, the lgtm code index is
...
// "the chunk body". `content` is the console code index, `text` is every lgtm
...
// error. The widths below are the ones lgtm and lgtm Cloud have actually written:
// 3072 is the current lgtm code index, 1536 is the console/Convex code index, and
...
func defaultEmbedderFactory(dimensions int) (reviewResourceEmbedder, string, bool) {
	...
	apiKey := strings.TrimSpace(firstNonEmpty(os.Getenv("OPENAI_API_KEY"), os.Getenv("LGTM_OPENAI_API_KEY")))
	if apiKey == "" {
		return nil, "", false
	}
...
func embedModelForDimensions(dimensions int) (string, bool) {
	...
	if override := strings.TrimSpace(os.Getenv("LGTM_REVIEW_INDEX_EMBED_MODEL")); override != "" {
