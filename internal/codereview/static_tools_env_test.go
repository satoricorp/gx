...
// reviewed checkout's processes. These are the real names a lgtm developer has
// exported (see internal/cloud/credentials.go and internal/telemetry) plus the
// two cloud conventions a CI runner exports into the step that runs lgtm review.
var reviewerCredentialEnv = map[string]string{
	...
	"GH_TOKEN":                       "gho-leaked",
	"LGTM_UPLOAD_TOKEN":          "lgtm-leaked",
	"LGTM_API_URL":               "https://internal.example.invalid",
...
func TestStaticToolChildEnvKeepsTheVariablesTheToolchainsNeed(t *testing.T) {
	...
	if got["GOCACHE"] != filepath.Join(os.TempDir(), "lgtm-review-gocache") {
...
// the reviewer's environment would be exfiltrating it through lgtm itself.
//
...
func TestFailingCheckoutTestsCarryNoReviewerCredentialsBackIntoTheReport(t *testing.T) {
	...
	for name, value := range reviewerCredentialEnv {
		if strings.Contains(output, "CHILD-ENV "+name+"=") {
			t.Fatalf("the reviewed checkout read %s and lgtm carried it back into the report:\n%s", name, output)
		}
...
func TestWhatTheReviewedCheckoutCanSee(t *testing.T) {
	for _, name := range []string{
		...
		"GITHUB_TOKEN", "GH_TOKEN", "LGTM_UPLOAD_TOKEN", "LGTM_API_URL",
...
// that lgtm captures the dump and carries it into the review report.
...
// command the static tool stage would run, with the environment lgtm itself has.
