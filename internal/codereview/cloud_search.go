...
// lgtm Cloud instead of raw provider keys.
//
...
// authenticates with `lgtm auth login` and retrieves through POST
// /v1/review/search, the same trade the /lgtm/openai and /lgtm/bedrock proxies
...
func cloudUnavailableDetail(err error) string {
	return "lgtm cloud retrieval failed: " + err.Error()
}
...
const signInRemedy = "Run `lgtm auth login` so review retrieval can use lgtm Cloud"
...
func retrieveCodeIndexViaCloud(
	...
	if repoFullName == "" {
		in.Evidence.Record(EvidenceStatus{
			...
			Detail: "this checkout has no GitHub remote, so lgtm Cloud has no repository to search",
	...
	case !result.Exists:
		...
		status.Detail = "lgtm Cloud has not indexed this repository"
	...
	status.Detail = joinDetail(note, "via lgtm cloud")
	in.Evidence.Record(status)
	return snippets, nil
}
