...
var staticToolEnvAllowed = buildStaticToolEnvAllowlist(
	...
	// redirects XDG_CACHE_HOME expects gx to honour it like everything else.
	...
	// script, so a gx invoked from an npm script would leak the registry token
	// under a prefix rule.
...
func staticToolChildEnv(parent []string) []string {
	...
	// day job depends on.
	values["GOCACHE"] = filepath.Join(os.TempDir(), "gx-review-gocache")
