...
// runs `tl review`. A TestMain that prints os.Environ() exfiltrates all of it,
...
var staticToolEnvAllowed = buildStaticToolEnvAllowlist(
	...
	// redirects XDG_CACHE_HOME expects tl to honour it like everything else.
	...
	// output when CI is set. tl review is a CI gate; let them know.
	"CI",
	...
	// script, so a tl invoked from an npm script would leak the registry token
	// under a prefix rule.
...
func staticToolChildEnv(parent []string) []string {
	...
	// day job depends on.
	values["GOCACHE"] = filepath.Join(os.TempDir(), "tl-review-gocache")
