...
// caller to guess which one lgtm obeyed.
...
// report it produces is published: `lgtm review` upserts it as the PR comment
// and records it as the lgtm Cloud history summary. So the wording is pinned
...
// `lgtm review --repo --fail-on any` on a clean tree reports the repository
...
// so adding one cannot move what lgtm writes on a PR.
