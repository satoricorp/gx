...
Data that came from a user, a request, a file, or another service must not be spliced into a query, a shell command, a URL, an HTML document, or a format string without the escaping that sink expects. This catches string-concatenated SQL, `exec.Command("sh", "-c", userInput)`, a template rendered with `template.HTML(input)`, and a path built by joining a request parameter without a root check. A constant string, a value already validated against an allowlist, or a parameter passed through a driver's placeholder mechanism does not count. Use the parameterized or escaping form the library provides, and validate at the boundary before the value reaches the sink.
