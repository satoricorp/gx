...
func renderTestReport() GatesReport {
	return GatesReport{
		RepoRoot:    "/tmp/repo",
		Reviewed:    true,
		ReviewMode:  ReviewModeRange,
		ReviewRange: "main...HEAD",
		DiffStats:   GatesDiffStats{Files: 2, AddedLines: 40, RemovedLines: 3},
...
func TestRenderGatesTextPlain(t *testing.T) {
	report := renderTestReport()
	report.Color = false
	out := RenderGatesText(report)
	if strings.Contains(out, "\x1b[") {
		t.Fatalf("plain render carries ANSI escapes:\n%s", out)
	}
	for _, want := range []string{
		"Gates check — main...HEAD (2 files, +40/−3)",
		"1. Correctness",
		"PASS",
		"FAIL",
...
func TestRenderGatesTextVerboseListsFiles(t *testing.T) {
	report := renderTestReport()
	report.Verbose = true
	out := RenderGatesText(report)
	if !strings.Contains(out, "files: a.go") {
...
func TestRenderGatesTextShipVariant(t *testing.T) {
	report := renderTestReport()
...
func TestRenderGatesTextNothingToCheck(t *testing.T) {
	report := GatesReport{Verdict: VerdictNothingToCheck, ReviewTarget: "the working tree"}
	out := RenderGatesText(report)
...
func TestRenderGatesMarkdownTable(t *testing.T) {
	report := renderTestReport()
	out := RenderGatesMarkdown(report)
	for _, want := range []string{
		"## Gates check — main...HEAD (2 files, +40/−3)",
...
func TestRenderGatesShowsDiffHunks(t *testing.T) {
	...
	if md := RenderGatesMarkdown(report); !strings.Contains(md, "@@ -0,0 +1,3 @@") {
