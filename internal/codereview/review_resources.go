...
const (
	defaultReviewKnowledgeNamespace = "lgtm-review-knowledge"
...
func reviewResourceRetrieverFromEnv() ContextRetriever {
	if strings.EqualFold(strings.TrimSpace(os.Getenv("LGTM_REVIEW_RESOURCES")), "0") {
		return nil
	}
	retriever := ReviewResourceRetriever{
		Limit: reviewEnvInt("LGTM_REVIEW_RESOURCES_TOP_K", defaultReviewResourceTopK),
	}
	openAIKey := strings.TrimSpace(firstNonEmpty(os.Getenv("OPENAI_API_KEY"), os.Getenv("LGTM_OPENAI_API_KEY")))
	...
	if openAIKey == "" || tpufKey == "" {
		...
		// through lgtm Cloud when signed in, and otherwise records the source as
	...
	namespace := strings.TrimSpace(os.Getenv("LGTM_REVIEW_KNOWLEDGE_NAMESPACE"))
	if namespace == "" {
		namespace = defaultReviewKnowledgeNamespace
	}
	cfg := semantic.Config{
		OpenAIAPIKey:         openAIKey,
		OpenAIBaseURL:        normalizeReviewOpenAIBaseURL(firstNonEmpty(os.Getenv("LGTM_OPENAI_BASE_URL"), os.Getenv("OPENAI_BASE_URL"), "https://api.openai.com")),
		OpenAIEmbeddingModel: firstNonEmpty(os.Getenv("LGTM_OPENAI_EMBEDDING_MODEL"), "text-embedding-3-small"),
		...
		EmbeddingDimensions: reviewEnvInt("LGTM_EMBEDDING_DIMENSIONS", 512),
	}
	retriever.Embedder = semantic.NewOpenAIEmbedder(cfg)
	retriever.Store = turboPufferReviewResourceStore{
		apiKey:     tpufKey,
		baseURL:    strings.TrimRight(firstNonEmpty(os.Getenv("LGTM_TPUF_BASE_URL"), defaultReviewResourceBaseURL), "/"),
...
// through lgtm Cloud. The server embeds the query at the corpus's own width and
...
func (r ReviewResourceRetriever) retrieveKnowledgeViaCloud(
	...
	case !result.Available:
		...
		status.Detail = "lgtm cloud retrieval is not configured server-side"
		if result.Reason != "" {
			status.Detail = "lgtm cloud: " + result.Reason
		}
	case !result.Exists:
...
func (r ReviewResourceRetriever) Retrieve(ctx context.Context, in RetrieveInput) ([]ContextSnippet, error) {
	...
	if r.Embedder == nil || r.Store == nil {
		...
		// through lgtm Cloud, and with no login the source reports itself
		...
		if searcher == nil {
			in.Evidence.Record(EvidenceStatus{
				...
				Detail: "not signed in to lgtm Cloud, and no direct keys for the shared review corpus",
				Remedy: signInRemedy,
			})
			return nil, nil
		}
...
func (s turboPufferReviewResourceStore) Query(ctx context.Context, req reviewResourceQuery) ([]reviewResourceRow, error) {
	...
	// lgtm-review-knowledge has no `publisher` column, so every review resource
...
func reviewResourceQueryText(opts Options, signals reviewResourceSignalSet) string {
	parts := []string{
		"lgtm code review resource query",
