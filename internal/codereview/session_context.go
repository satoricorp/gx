...
const (
	...
	// itself.
	legacySessionNamespace = "lgtm-sessions-dev"
)
...
func sessionContextRetrieverFromEnv() ContextRetriever {
	if strings.EqualFold(strings.TrimSpace(os.Getenv("LGTM_REVIEW_SESSION_CONTEXT")), "0") {
		return nil
	}
	return SessionContextRetriever{
		Limit: reviewEnvInt("LGTM_REVIEW_SESSION_CONTEXT_TOP_K", defaultSessionContextTopK),
	}
}
...
func (r SessionContextRetriever) Retrieve(ctx context.Context, in RetrieveInput) ([]ContextSnippet, error) {
	...
	store := r.Store
	if store == nil {
		// The signed-in path: retrieval through lgtm Cloud, no provider keys on
		...
		store = newTurboPufferIndexStore(strings.TrimSpace(os.Getenv("TURBOPUFFER_API_KEY")), firstNonEmpty(os.Getenv("LGTM_TPUF_BASE_URL"), defaultReviewResourceBaseURL))
	}
...
func sessionNamespaceTargets(repoRoot, repoFullName string) []codeIndexTarget {
	if override := strings.TrimSpace(os.Getenv("LGTM_REVIEW_SESSION_NAMESPACE")); override != "" {
		var out []codeIndexTarget
		for _, name := range strings.Split(override, ",") {
			if name = strings.TrimSpace(name); name != "" {
				out = append(out, codeIndexTarget{Namespace: name, Origin: "LGTM_REVIEW_SESSION_NAMESPACE"})
			}
		}
		return out
	}
	// Same resolver as the code index and as `lgtm index`: publish artifacts,
...
func sessionQueryText(in RetrieveInput) string {
	...
	return strings.Join([]string{
		"lgtm code review: recall the agent sessions behind this code.",
