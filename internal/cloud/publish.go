...
func (c *Client) RegisterPublish(ctx context.Context, registration PublishRegistration) (PublishRegistrationResult, error) {
	...
	if publishURL == "" {
		return PublishRegistrationResult{}, fmt.Errorf("gx cloud base URL is not configured")
	}
	body, err := json.Marshal(registration)
	if err != nil {
		return PublishRegistrationResult{}, fmt.Errorf("marshal gx publish registration: %w", err)
	}
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, publishURL, bytes.NewReader(body))
	if err != nil {
		return PublishRegistrationResult{}, fmt.Errorf("create gx publish registration request: %w", err)
	}
	...
	if err != nil {
		return PublishRegistrationResult{}, fmt.Errorf("register gx publish: %w", err)
	}
	...
	if resp.StatusCode < 200 || resp.StatusCode > 299 {
		...
		if detail != "" {
			return PublishRegistrationResult{}, fmt.Errorf("register gx publish: status %s: %s", resp.Status, detail)
		}
		return PublishRegistrationResult{}, fmt.Errorf("register gx publish: status %s", resp.Status)
	}
	var result PublishRegistrationResult
	if err := json.Unmarshal(raw, &result); err != nil {
		return PublishRegistrationResult{}, fmt.Errorf("decode gx publish registration: %w", err)
	}
	return result, nil
}
