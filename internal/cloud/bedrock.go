...
// defaultBedrockTimeout matches the server's own ceiling on /tl/bedrock/fight.
...
// BedrockFightRequest is the request /tl/bedrock/fight accepts.
//
...
func (c *Client) BedrockFight(ctx context.Context, reqBody BedrockFightRequest) (BedrockFightResponse, error) {
	...
	if c == nil || c.url == "" {
		return result, fmt.Errorf("tl cloud base URL is not configured")
	}
	if err := c.postJSON(ctx, "/tl/bedrock/fight", reqBody, &result); err != nil {
