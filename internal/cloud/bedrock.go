...
// defaultBedrockTimeout matches the server's own ceiling on /lgtm/bedrock/fight.
...
// BedrockFightRequest is the request /lgtm/bedrock/fight accepts.
//
...
func bedrockTimeout() time.Duration {
	raw := strings.TrimSpace(os.Getenv("LGTM_CLOUD_BEDROCK_TIMEOUT"))
	if raw == "" {
...
// BedrockFight runs one model call on lgtm's Bedrock account.
//
// lgtm holds the AWS credentials, so a user with no AWS account still gets the
...
func (c *Client) BedrockFight(ctx context.Context, reqBody BedrockFightRequest) (BedrockFightResponse, error) {
	...
	if c == nil || c.url == "" {
		return result, fmt.Errorf("lgtm cloud base URL is not configured")
	}
	if err := c.postJSON(ctx, "/lgtm/bedrock/fight", reqBody, &result); err != nil {
