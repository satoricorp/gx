...
const (
	gxStackBookmarkPrefix       = "gx/"
	legacyGXDraftBookmarkPrefix = "gx/draft/"
	gxInternalBaseRef           = "gx/base"
	gxInternalEditRef           = "gx/edit"
)

func isGXStackBookmark(name string) bool {
	name = strings.TrimSpace(name)
	if isGXInternalCheckoutRef(name) {
		return false
	}
	return strings.HasPrefix(name, gxStackBookmarkPrefix)
}
...
func gxAuthoringBaseFromCheckoutRef(name string) (string, bool) {
	name = strings.TrimSpace(name)
	if name == "" || name == gxInternalEditRef || name == gxInternalBaseRef {
		return "", false
	}
	if !strings.HasPrefix(name, gxStackBookmarkPrefix) {
		return "", false
	}
	base := strings.TrimPrefix(name, gxStackBookmarkPrefix)
	if base == "" || strings.HasPrefix(base, "draft/") {
		return "", false
	}
	return base, true
}
...
func stackBookmarkName(name, headChangeID string) string {
	bookmark := gxStackBookmarkPrefix + bookmarkSlug(name)
	if strings.TrimSuffix(bookmark, gxStackBookmarkPrefix) == "" {
		bookmark = gxStackBookmarkPrefix + shortID(headChangeID, 8)
	}
	return bookmark
}
...
func stackNameFromBookmark(name string) string {
	...
	name = strings.TrimPrefix(name, gxStackBookmarkPrefix)
	return name
}

func legacyStackBookmarkName(name string) bool {
	return strings.HasPrefix(strings.TrimSpace(name), legacyGXDraftBookmarkPrefix)
}
...
func (s *Service) normalizeLegacyStackBookmarks(ctx context.Context, store *storage.Store, repoRoot string, repoID int64) error {
	...
	for _, stack := range stacks {
		if legacyStackBookmarkName(stack.BaseRef) {
			newBaseRef := gxStackBookmarkPrefix + stackNameFromBookmark(stack.BaseRef)
		...
		newName := gxStackBookmarkPrefix + normalizedName
		if newName == gxStackBookmarkPrefix {
			continue
		}
