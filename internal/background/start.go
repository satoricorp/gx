...
// StartDetachedLgtm launches the lgtm binary with args in a detached background process.
//
// It refuses to launch anything that is not the lgtm binary. os.Executable()
...
// LGTM_DISABLE_BACKGROUND_WORKERS; this guard is for the ones that forget.
func StartDetachedLgtm(args ...string) error {
	exe, err := os.Executable()
	if err != nil {
		return err
	}
	if runningUnderGoTest(exe) {
		return fmt.Errorf("refusing to spawn %q: not the lgtm binary", filepath.Base(exe))
	}
