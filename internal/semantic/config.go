...
const (
	...
	// "lgtm-sessions", is a flat global namespace with no org and no repo
...
func ConfigFromEnv() (Config, error) {
	enabled := truthy(os.Getenv("LGTM_SEMANTIC_INDEX")) || strings.TrimSpace(os.Getenv("LGTM_TPUF_NAMESPACE")) != ""
	cfg := Config{
		Enabled:              enabled,
		OpenAIAPIKey:         strings.TrimSpace(os.Getenv("OPENAI_API_KEY")),
		OpenAIBaseURL:        envOrDefault("LGTM_OPENAI_BASE_URL", defaultOpenAIBaseURL),
		OpenAIEmbeddingModel: envOrDefault("LGTM_OPENAI_EMBEDDING_MODEL", defaultOpenAIEmbedModel),
		EmbeddingDimensions:  envIntOrDefault("LGTM_EMBEDDING_DIMENSIONS", defaultEmbeddingDims),
		TurboPufferAPIKey:    strings.TrimSpace(os.Getenv("TURBOPUFFER_API_KEY")),
		TurboPufferBaseURL:   envOrDefault("LGTM_TPUF_BASE_URL", defaultTurboPufferBaseURL),
		TurboPufferNamespace: envOrDefault("LGTM_TPUF_NAMESPACE", defaultTurboPufferNS),
		BatchSize:            envIntOrDefault("LGTM_SEMANTIC_BATCH_SIZE", defaultBatchSize),
		MaxChunkBytes:        envIntOrDefault("LGTM_SEMANTIC_MAX_CHUNK_BYTES", defaultMaxChunkBytes),
	}
	if !cfg.Enabled {
		return cfg, nil
	}
	...
	if cfg.EmbeddingDimensions <= 0 {
		return cfg, fmt.Errorf("LGTM_EMBEDDING_DIMENSIONS must be positive")
	}
	if cfg.BatchSize <= 0 {
		return cfg, fmt.Errorf("LGTM_SEMANTIC_BATCH_SIZE must be positive")
	}
	if cfg.MaxChunkBytes <= 0 {
		return cfg, fmt.Errorf("LGTM_SEMANTIC_MAX_CHUNK_BYTES must be positive")
	}
	if _, err := url.ParseRequestURI(strings.TrimRight(cfg.OpenAIBaseURL, "/")); err != nil {
		return cfg, fmt.Errorf("LGTM_OPENAI_BASE_URL is invalid: %w", err)
	}
	if _, err := url.ParseRequestURI(strings.TrimRight(cfg.TurboPufferBaseURL, "/")); err != nil {
		return cfg, fmt.Errorf("LGTM_TPUF_BASE_URL is invalid: %w", err)
	}
	...
	// which silently pointed every read and write at "lgtm-sessions" — a
	// namespace that has never existed.
	if cfg.TurboPufferNamespace == "" {
		return cfg, fmt.Errorf("LGTM_TPUF_NAMESPACE is required when LGTM_SEMANTIC_INDEX is set; repository indexing resolves its own namespace via semantic.NamespaceForRepo")
	}
	return cfg, nil
}
...
//
// Unlike ConfigFromEnv it is not behind LGTM_SEMANTIC_INDEX. Repository indexing
// is the thing that makes review retrieval work at all, so it runs whenever
// credentials exist; LGTM_SEMANTIC_INDEX=0 still turns it off explicitly. It also
...
func CodeIndexConfigFromEnv() Config {
	model := envOrDefault("LGTM_OPENAI_EMBEDDING_MODEL", defaultOpenAIEmbedModel)
	dimensions := envIntOrDefault("LGTM_EMBEDDING_DIMENSIONS", defaultEmbeddingDims)
	if max := MaxEmbeddingDimensions(model); max > 0 && (dimensions <= 0 || dimensions > max) {
		dimensions = max
	}
	return Config{
		Enabled:              !strings.EqualFold(strings.TrimSpace(os.Getenv("LGTM_SEMANTIC_INDEX")), "0"),
		OpenAIAPIKey:         strings.TrimSpace(firstNonEmptyString(os.Getenv("OPENAI_API_KEY"), os.Getenv("LGTM_OPENAI_API_KEY"))),
		OpenAIBaseURL:        envOrDefault("LGTM_OPENAI_BASE_URL", defaultOpenAIBaseURL),
...
//
// The shape matches the console writer (`lgtm-<orgId>-<slug>`) so the CLI and the
...
// `lgtm-local-` namespace so a first review still has an index to read.
