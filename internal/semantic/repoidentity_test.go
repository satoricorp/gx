...
// `tl index` wrote totality-local-yeet-8d862445e7e4-v2 while `tl review` in the same
...
func TestIndexAndReviewResolveTheSameNamespace(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if written != read {
				t.Fatalf("`tl index` writes %q but `tl review` reads %q — a review of this repository would report it as never indexed", written, read)
			}
