...
// installTotalityPath resolves the tx binary to pin into a hook script, and declines
// to pin one that lives in a temporary directory.
//
// Hooks pin the install-time path and fall back to tx on PATH when it is gone.
...
//
// Returning empty means the hook resolves tx from PATH every time, which is
