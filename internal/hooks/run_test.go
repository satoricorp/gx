...
func TestRunPushRespectsPauseFlag(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(lgtmHome, "pause-capture"), []byte("1\n"), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestRunPushRecoversMissingRevision(t *testing.T) {
	...
	repo, err := service.ResolveLgtmRepoAtPath(context.Background(), repoRoot)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRunPushReportsRevisionScanFailure(t *testing.T) {
	...
	if !strings.Contains(outcome.RecoveryError, "scan lgtm revision trailers") {
...
// shareable-marking pass is most likely to hit in the field: ~/.lgtm/lgtm.db is WAL
// with a 5s busy timeout and the detached `lgtm capture sync` spawned by the
...
// capture and a repo opted out with `git config lgtm.enabled false` both returned
...
func TestRunPushNamesEveryReasonItDidNothing(t *testing.T) {
	...
	t.Run("repo opted out", func(t *testing.T) {
		...
		initGitRepo(t, repo)
		t.Setenv("LGTM_HOME", t.TempDir())
		runGitInHookTest(t, repo, "config", "lgtm.enabled", "false")

		outcome, err := RunPush(context.Background(), PushOptions{RepoRoot: repo, HomeDir: home})
		if err != nil {
			t.Fatal(err)
		}
		if !strings.Contains(outcome.SkipReason, "lgtm.enabled") {
...
// failing upload can reach a human. `lgtm capture sync` runs detached with stdout
