...
// installLgtmPath resolves the lgtm binary to pin into a hook script, and declines
// to pin one that lives in a temporary directory.
//
// Hooks pin the install-time path and fall back to lgtm on PATH when it is gone.
...
//
// Returning empty means the hook resolves lgtm from PATH every time, which is
// the correct behavior when the running binary has no durable location.
func installLgtmPath() (string, error) {
