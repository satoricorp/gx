...
// on a brand new database, for every push from a worktree. lgtm's own
...
func TestRunPushFromLinkedWorktreePublishesSessions(t *testing.T) {
	...
	if _, err := vcs.NewService().ResolveLgtmRepoAtPath(ctx, main); err != nil {
		t.Fatal(err)
	}
...
// commitWithTrailer writes files and commits them with a lgtm revision trailer,
