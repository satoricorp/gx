...
func refreshGitHubAccessToken(refreshToken string, client *http.Client) (accessTokenResponse, error) {
	...
	endpoint := strings.TrimSpace(os.Getenv("LGTM_GITHUB_ACCESS_TOKEN_URL"))
	if endpoint == "" {
...
func reloginError(reason string) error {
	...
	if strings.TrimSpace(os.Getenv("LGTM_MCP")) != "" {
		return fmt.Errorf("%s; run `lgtm auth login` in a terminal, then retry the MCP tool", reason)
	}
	return fmt.Errorf("%s; run `lgtm auth login` to refresh stored credentials or set GH_TOKEN/GITHUB_TOKEN", reason)
}
