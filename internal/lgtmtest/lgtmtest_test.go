...
func TestNewRepoIsAMainCheckout(t *testing.T) {
	repo := lgtmtest.NewWorld(t).NewRepo(t)
...
// repository's common dir. Repository identity in lgtm is that shared common dir,
...
// this harness depends on: without the lgtm trailer no `changes` row is created,
...
func TestCommitStampsATrailer(t *testing.T) {
	...
	message := lgtmtest.GitOutput(t, repo.Root, "log", "-1", "--pretty=%B")
	if want := lgtmtest.RevisionTrailerLine(commit.RevisionID); !strings.Contains(message, want) {
