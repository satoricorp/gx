// Package storagetest builds lgtm databases that look like the ones on real
...
// the same reason: every test did `t.Setenv("LGTM_HOME", t.TempDir())` against a
...
//     repository: an old one with root_path=/Users/joe/git/lgtm and
//     git_common_dir=/Users/joe/git/lgtm (the pre-git_common_dir backfill shape,
//     48% of live rows), and a newer one with root_path="" and
//     git_common_dir=/Users/joe/git/lgtm/.git carrying every recent change. The
...
//
// Layering. This package imports internal/storage and internal/lgtmtest and
// nothing else from lgtm. internal/vcs imports internal/storage, so importing
...
// `lgtm commit` self-report. storage.Open deletes it and its links on every open;
// nothing else in the tree covers that migration.
const CommitSelfReportSessionID = "lgtm-commit-self-report"

// Harness is an open lgtm database plus the ids the shapes created in it.
type Harness struct {
...
// New opens an isolated lgtm database in its own lgtmtest.World and applies shapes.
func New(t *testing.T, shapes ...Shape) *Harness {
	t.Helper()
	return NewInWorld(t, lgtmtest.NewWorld(t), shapes...)
}

// NewInWorld opens the lgtm database belonging to an existing world, so a test
// can create its git repositories first and then shape the database around
// their real paths.
func NewInWorld(t *testing.T, world *lgtmtest.World, shapes ...Shape) *Harness {
	t.Helper()
...
//
//	id 1  root_path=/Users/joe/git/lgtm  git_common_dir=/Users/joe/git/lgtm
//	id 16 root_path=""                 git_common_dir=/Users/joe/git/lgtm/.git
//
...
// FossilCommitSelfReportSession writes the `lgtm-commit-self-report` sessions row
// an older lgtm binary left behind.
//
