...
func (c *Client) do(req *http.Request, out any) error {
	...
	if resp.StatusCode == http.StatusUnauthorized || resp.StatusCode == http.StatusForbidden {
		...
		if detail != "" {
			...
			return fmt.Errorf("github auth failed: status %s: %s. Run `tx auth login` to refresh stored credentials or set GH_TOKEN/GITHUB_TOKEN", resp.Status, detail)
		}
		return fmt.Errorf("github auth failed: status %s. Run `tx auth login` to refresh stored credentials or set GH_TOKEN/GITHUB_TOKEN", resp.Status)
	}
