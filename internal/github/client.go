...
func tokenError() error {
	if strings.TrimSpace(os.Getenv("LGTM_MCP")) != "" {
		return fmt.Errorf("github token is not configured for MCP: run `lgtm auth login` in a terminal, then retry the MCP tool")
	}
	return fmt.Errorf("github token is not configured: run `lgtm auth login` or set GH_TOKEN/GITHUB_TOKEN")
}
...
func apiBaseURL(host string) string {
	if override := strings.TrimSpace(os.Getenv("LGTM_GITHUB_API_URL")); override != "" {
...
func (c *Client) do(req *http.Request, out any) error {
	...
	if resp.StatusCode == http.StatusUnauthorized || resp.StatusCode == http.StatusForbidden {
		...
		if detail != "" {
			...
			return fmt.Errorf("github auth failed: status %s: %s. Run `lgtm auth login` to refresh stored credentials or set GH_TOKEN/GITHUB_TOKEN", resp.Status, detail)
		}
		return fmt.Errorf("github auth failed: status %s. Run `lgtm auth login` to refresh stored credentials or set GH_TOKEN/GITHUB_TOKEN", resp.Status)
	}
