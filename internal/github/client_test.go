...
func TestFindPullRequestUsesGitHubAPI(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path != "/repos/satoricorp/lgtm/pulls" {
			t.Fatalf("path = %q, want /repos/satoricorp/lgtm/pulls", r.URL.Path)
		}
		if r.URL.Query().Get("head") != "satoricorp:feature/demo" {
			t.Fatalf("head query = %q", r.URL.Query().Get("head"))
		}
		_, _ = w.Write([]byte(`[{"number":7,"html_url":"https://github.com/satoricorp/lgtm/pull/7","body":"existing body"}]`))
	}))
	defer server.Close()
	...
	if pr == nil || pr.URL != "https://github.com/satoricorp/lgtm/pull/7" || pr.Number != 7 || pr.Body != "existing body" {
...
func TestFindPullRequestByHeadCanIncludeMergedPRs(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`[{"number":7,"html_url":"https://github.com/satoricorp/lgtm/pull/7","state":"closed","merged_at":"2026-06-28T12:00:00Z"}]`))
	}))
	defer server.Close()
	t.Setenv("LGTM_GITHUB_API_URL", server.URL)
...
func TestUpsertIssueCommentCreatesWhenMissing(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch {
		case r.Method == http.MethodGet && r.URL.Path == "/repos/satoricorp/lgtm/issues/8/comments":
			_, _ = w.Write([]byte(`[]`))
		case r.Method == http.MethodPost && r.URL.Path == "/repos/satoricorp/lgtm/issues/8/comments":
			...
			_, _ = w.Write([]byte(`{"id":12,"html_url":"https://github.com/satoricorp/lgtm/pull/8#issuecomment-12","body":"new body"}`))
		default:
	...
	if got := strings.Join(methods, ","); got != "GET /repos/satoricorp/lgtm/issues/8/comments,POST /repos/satoricorp/lgtm/issues/8/comments" {
...
func TestUpsertIssueCommentUpdatesExistingMarker(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case r.Method == http.MethodGet && r.URL.Path == "/repos/satoricorp/lgtm/issues/8/comments":
			_, _ = w.Write([]byte(`[{"id":22,"html_url":"https://github.com/satoricorp/lgtm/pull/8#issuecomment-22","body":"old\n<!-- marker -->"}]`))
		case r.Method == http.MethodPatch && r.URL.Path == "/repos/satoricorp/lgtm/issues/comments/22":
			patched = true
			var payload map[string]string
			if err := json.NewDecoder(r.Body).Decode(&payload); err != nil {
				t.Fatalf("decode body: %v", err)
			...
			_, _ = w.Write([]byte(`{"id":22,"html_url":"https://github.com/satoricorp/lgtm/pull/8#issuecomment-22","body":"replacement"}`))
		default:
			t.Fatalf("unexpected request %s %s", r.Method, r.URL.Path)
		}
	}))
...
func TestCreatePullRequestReviewCommentUsesGitHubAPI(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path != "/repos/satoricorp/lgtm/pulls/8/comments" {
			t.Fatalf("path = %q, want /repos/satoricorp/lgtm/pulls/8/comments", r.URL.Path)
		}
		if err := json.NewDecoder(r.Body).Decode(&payload); err != nil {
			t.Fatalf("decode body: %v", err)
		}
...
func TestGitHubAuthError(t *testing.T) {
	...
	_, err := client.FindPullRequest(context.Background(), CreatePullRequestOptions{Owner: "satoricorp", Repo: "lgtm", HeadBranch: "feature/demo"})
...
func TestNewClientUsesStoredGitHubToken(t *testing.T) {
  ...
  "cloud": {
    "token": "lgtm_saved_token_with_enough_length",
