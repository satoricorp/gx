...
func TestSyncPendingReadsSourceFiles(t *testing.T) {
	...
	// The wire format is lgtm's own line-oriented transcript, never the raw
...
//   - content is lgtm's line format, because the server promoter parses only
...
func TestSyncPendingCapsSessionContent(t *testing.T) {
	...
	if !strings.Contains(uploaded, "[lgtm] truncated") {
