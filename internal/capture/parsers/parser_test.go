...
func TestDiscoverFindsCursorAgentTranscripts(t *testing.T) {
	home := t.TempDir()
	repoRoot := filepath.Join(string(filepath.Separator), "Users", "joe", "git", "lgtm")
	root := filepath.Join(home, ".cursor", "projects", "Users-joe-git-lgtm", "agent-transcripts", "parent-1")
	mainPath := filepath.Join(root, "parent-1.jsonl")
	subPath := filepath.Join(root, "subagents", "sub-1.jsonl")
	oldPath := filepath.Join(home, ".cursor", "projects", "Users-joe-git-lgtm", "agent-transcripts", "old", "old.jsonl")
...
func TestDiscoverFindsClaudeSessionsFiledUnderAnotherProject(t *testing.T) {
	home := t.TempDir()
	repoRoot := filepath.Join(string(filepath.Separator), "lgtm-xrepo", "repo")
	sibling := filepath.Join(string(filepath.Separator), "lgtm-xrepo", "repo-cloud")
	other := filepath.Join(string(filepath.Separator), "lgtm-xrepo", "elsewhere")
...
func TestDiscoverSurvivesUnreadableCursorTranscripts(t *testing.T) {
	home := t.TempDir()
	repoRoot := filepath.Join(string(filepath.Separator), "lgtm-nocursor", "repo")
	now := time.Now()
...
func TestDiscoverReportsUnreadableClaudeAndCodexRoots(t *testing.T) {
	home := t.TempDir()
	repoRoot := filepath.Join(string(filepath.Separator), "lgtm-unreadable", "repo")
	now := time.Now()
...
func TestDiscoverStaysInsideRequestedHome(t *testing.T) {
	home := t.TempDir()
	repoRoot := filepath.Join(string(filepath.Separator), "lgtm-empty-home", "repo")
	now := time.Now()
...
func TestDiscoverAcceptsMultiGigabyteCursorDatabase(t *testing.T) {
	home := t.TempDir()
	repoRoot := filepath.Join(string(filepath.Separator), "lgtm-bigcursor", "repo")
	now := time.Now()
