...
- When `T`is not`Sized`the story is a little tricker:- If you have a reference `r`you can use`size_of_val(r)`to determine the span of the reference.
...
Zero Sized Types *never* alias each other, because their span length is always 0
bytes.
...
A *place* (called "lvalue" in C and "glvalue" in C++) is the result of computing a *place expression*.
...
A *value* (called "value of the expression" or "rvalue" in C and "prvalue" in C++) is what gets stored in a place, and also the result of computing a *value expression*.
...
`enum` types**This page has been archived**
