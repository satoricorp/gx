...
‡Corresponding author is Nan Duan.
...
(ESEC/FSE ’22), November 14–18, 2022, Singapore, Singapore. ACM, New York,
...
every month (~22k reviews in case of Qt). However, it’s not free
arXiv:2203.09095v2 [cs.SE] 11 Oct 2022
...
from both reviewers’ and committers’ perspectives. The first is
...
decoder transformer model. Different from Tufano et al. [40]’s work,
...
• The first pre-trained model that takes code diffs as input in
the code review scenario.
• Novel pre-training tasks for better code changes understand-
ing and generation.
• A large-scale code review related pre-training dataset and
a high-quality benchmark dataset for evaluation in nine
programming languages.
• A comprehensive evaluation of CodeReviewer and previous
SOTA models. Our dataset, code, and model are released 1.
2
CODE REVIEW AUTOMATION TASKS
...
as 𝐶0 and 𝐶1. Once the code changes (𝐷: 𝐶0 →𝐶1) are ready for
...
revisions and provides a newer version of code 𝐶2. We call the
...
roles are involved – contributor 𝑃𝐶who commits a code change
(𝐷: 𝐶0 →𝐶1), and reviewer 𝑃𝑅who gives advice and comments
(𝑅𝑛𝑙). The goal of code review automation is to help reduce their
...
needed
Need review
I think "import *" is not
allowed in Kylin's static
code analysis.
Review generation
Code refinement
...
is a code change, i.e., X = {𝐷(𝐶0,𝐶1)}. As stated before, there are
...
a predicted review comment, i.e., Y = {𝑤1, · · · ,𝑤𝑛} where 𝑤𝑖is a
natural language word and 𝑛∈N is the length of review comment.
The input is still a code change, i.e., X = {𝐷(𝐶0,𝐶1)}, with its
...
It’s not recommended for reviewers to give suggestions to the code
...
“This method should be private” to suggest the contributor add a
...
original code 𝐶1 written by the contributor and the review com-
ment 𝑅𝑛𝑙from the reviewer, and targets to generate the revised
version code 𝐶2 implementing the requirements mentioned in 𝑅𝑛𝑙.
...
with “-” and “+” tags at the begging of each line. In the first two
tasks, we formulate the input as a diff hunk representing the code
change. In the code refinement task, we extract the input lines (𝐶1)
and output lines (𝐶2) from the revision diff hunk.
3
CODE REVIEW DATASET
...
Heumüller et al. [20] develop the ETCR infrastructure for mining
...
the reviewer writes a comment on the code diff 𝐷: 𝐶0 →𝐶1 and
the revised source code lines in 𝐶1 are further modified to a newer
version 𝐶2 in a later commit, we assume that the comments on the
early change helped the contributor to make the later updates. Then
the triplets (𝐶1, 𝑅𝑛𝑙,𝐶2) are collected to build the code refinement
...
ment to the tokens. A special token [𝐶𝐿𝑆] is prepended to the
sequence, forming the input as {[𝐶𝐿𝑆],𝑐1,𝑐2, · · · ,𝑐𝑛} where 𝑐𝑖
is source code token and 𝑛is the sequence length. To help our
model understand the diff format better, the special line tags “-”
and “+” in diff file indicating line deletion and line insertion are
replaced with special tokens [𝐷𝐸𝐿] and [𝐴𝐷𝐷]. We also insert a
[𝐾𝐸𝐸𝑃] before each unchanged line. When there are both source
code tokens and review comment tokens in the input, a [𝑀𝑆𝐺]
token is inserted to separate the two sequences. Thus the input is
{[𝐶𝐿𝑆],𝑐1,𝑐2, · · · ,𝑐𝑛, [𝑀𝑆𝐺],𝑤1,𝑤2, · · · ,𝑤𝑚}.
The model outputs consist of two components: token represen-
tations from the encoder and generated token sequence from the
decoder. For classification tasks, the representation of [𝐶𝐿𝑆] token
...
4.3.1
Diff Tag Prediction. As mentioned in Section 2.5, diff is a
...
0 , 𝑝(𝑖)
1 , 𝑝(𝑖)
2 ) for the 𝑖-th masked tag position and a
...
(1)
4.3.2
Denoising Objective. Denoising objective is an unsupervised
...
of code diff. It’s worth noting that to reduce the difficulty of this
task, the line tags ([𝐴𝐷𝐷], [𝐷𝐸𝐿] and [𝐾𝐸𝐸𝑃]) are preserved to
...
ment, and c<𝑡, w<𝑡are the span sequence of code diff and review
comment generated so far.
4.3.3
Review Comment Generation. In each pre-training task men-
...
−log 𝑃(𝑤𝑡|c, w<𝑡)
(4)
where c is code change and w<𝑡is the comment generated so far.
4.4
Fine-Tuning
...
in the last layer of the special token [𝐶𝐿𝑆] at the beginning ℎ0 is
...
a code change 𝐷1 : 𝐶0 →𝐶1 , and ask the model to give a binary
...
change 𝐷1 : 𝐶0 →𝐶1, but we assess the ability of CodeReviewer to
generate a natural language comment 𝑅𝑛𝑙as the reviewers would
do.
...
4,311k
2,481k
† Pull request numbers of the projects in total.
...
𝑛-grams between the generated text and the reference text with 𝑛
...
6.1
RQ1: Performance on Code Change Quality
Estimation
...
6.2
RQ2: Performance on Review Generation
...
is lower than Codex. To investigate the models’ performance better,
...
comments.
In this task, we also conduct a qualitative analysis of the world’s
...
of zero-shot task transfer. Since we don’t get access to the model
and can’t fine-tune it, we alternately use GitHub Copilot [15] which
is powered by Codex. By providing several code diff and review
comment pairs as prompt, we find that Codex can’t generate any
...
T5: I don't think this change is needed.
BLEU:4.93
CodeT5: Why do we need this change?
BLEU:2.97
Codex: I think this is a good example of how to use the docker
BLEU:8.83
CodeReviewer: This is an important debugging help and shouldn't be lower than the default visible INFO.
BLEU:7.97
...
CCodeReviewer: return empty list instead?
...
6.3
RQ3: Performance on Code Refinement
Table 6 shows the results on the code refinement task. The Naïve-
...
6.4
RQ4: Influence of Pre-training Tasks
...
88.10
6.5
RQ5: Influence of Multilingual Dataset
...
the pre-training objectives of T5 [31] and don’t take into consid-
...
revise the contributors’ code to implement the requirements from
...
Betty H. C. Cheng, and Klaus Pohl (Eds.). IEEE Computer Society, 712–721.
https://doi.org/10.1109/ICSE.2013.6606617
[4] Moritz Beller, Alberto Bacchelli, Andy Zaidman, and Elmar Jürgens. 2014. Modern
...
Pinzger (Eds.). ACM, 202–211. https://doi.org/10.1145/2597073.2597082
[5] Amiangshu Bosu and Jeffrey C. Carver. 2013. Impact of Peer Code Review on Peer
Impression Formation: A Survey. In ESEM. IEEE Computer Society, 133–142.
...
(Eds.). Springer Berlin Heidelberg, 575–607. https://doi.org/10.1007/978-3-642-
59412-0_35
[13] Michael E. Fagan. 2002. A History of Software Inspections. In Software Pioneers.
Springer Berlin Heidelberg, 562–573.
...
Association for Computing Machinery, New York, NY, USA, 1479–1482. https:
//doi.org/10.1145/3468264.3473134
[20] Robert Heumüller, Sebastian Nielebock, and Frank Ortmeier. 2021. Exploit Those
...
2021). Association for Computing Machinery, New York, NY, USA, 1505–1509.
https://doi.org/10.1145/3468264.3473110
[21] Abram Hindle, Earl T. Barr, Mark Gabel, Zhendong Su, and Premkumar Devanbu.
2016. On the Naturalness of Software. Commun. ACM 59, 5 (apr 2016), 122–131.
...
(MSR ’18). Association for Computing Machinery, New York, NY, USA, 46–49.
...
2013, Bertrand Meyer, Luciano Baresi, and Mira Mezini (Eds.). ACM, 202–212.
...
Paulisch and Jan Bosch (Eds.). ACM, 181–190. https://doi.org/10.1145/3183519.
3183525
...
Automating Review Recommendation for Code Changes. In SANER. IEEE, 284–
295.
...
Computing Machinery, New York, NY, USA, 2291–2302. https://doi.org/10.1145/
3510003.3510621
...
Automating Code Review Activities by Large-Scale Pre-training
ESEC/FSE ’22, November 14–18, 2022, Singapore, Singapore
Spain, 22-30 May 2021. IEEE, 163–174. https://doi.org/10.1109/ICSE43902.2021.
00027
...
Christian Bird (Eds.). ACM, 460–463. https://doi.org/10.1145/2901739.2903504
...
on Software Engineering 42, 6 (2015), 530–543.
