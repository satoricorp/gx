...
- Use the phab-test-policy add-on to help select the right test policy for the patch in Phabricator.
...
- Error messages that appear in web platform environments should explain the reason for the error, and use web platform terminology (as opposed to internal Firefox terminology). More details can be found in the helpful error messages guide.
...
- If the patch is simple enough and has no linter errors, the reviewer should not hesitate to land it directly after approving it, rather than leaving it to the author. This reduces round-trips and gets the fix into the tree faster, which is especially helpful for new contributors.
