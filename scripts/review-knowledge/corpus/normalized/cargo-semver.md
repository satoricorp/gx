...
- When introducing the enum, mark the variant as `non_exhaustive`so that it cannot be constructed or matched without wildcards.`pub enum E { #[non_exhaustive] Variant1{f1: i32} }`
...
`+ use<…>`specified) or explicitly captured (must be listed in`+ use<…>`), and thus currently it is not allowed to change what is captured of those kinds of generics.
...
`package.rust-version`, allowing Cargo’s dependency resolution to attempt to select older versions of your package when needed. Be sure to consider the support expectations when doing so.
