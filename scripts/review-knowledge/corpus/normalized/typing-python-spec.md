...
(See `613` for the introduction of `TypeAlias`, and
`695` for the `type` statement.)
...
See also `protocol-newtype-aliases` for a discussion of how
...
The type system leverages `3107`-style annotations with a number of
...
Expressions whose type is `assignable` to a specific argument type are
...
Any function without annotations can be treated as having `Any`
...
* The definitions of fields in the functional forms for creating
 TypedDict and NamedTuple types
* The value passed to the `extra_items` argument in the TypedDict
 constructor
...
is a type expression, optionally surrounded by one or more type qualifiers
...
exceptions at the time the function is defined (but see `forward-references`).
...
* `<Name>` refers to a `special form`. Most special forms must be imported
 from `typing` or `typing_extensions`, except for `None`, `InitVar`,
 `type`, and `tuple`. The latter two have aliases in `typing`: `typing.Type`
 and `typing.Tuple`. `InitVar` must be imported from `dataclasses`.
 `Callable` may be imported from either `typing` or `collections.abc`.
...
it is constrained by the rules from the expression grammar.
...
Coroutines introduced in `492` are annotated with the same syntax as
...
Support for the `/` delimiter was introduced in Python 3.8 (`570`).
...
evaluated, a type checker should verify that this type is `assignable` to
...
* As introduced by `646`, a backward-compatible form for certain operations
 involving variadic generics. See the section on `TypeVarTuple` for details.
* As introduced by `692`, a way to annotate the `**kwargs` of a function.
...
(Originally specified in `526`.)

The `typing.ClassVar` `type qualifier` is used to annotate
...
`~typing.ClassVar` may be used in one of several forms:
...
Here `ClassVar` is a `special form` defined by the `typing` module that
...
`ClassVar` cannot be used as a qualifier for a TypedDict
item or a NamedTuple field. Such usage also generates
an error at runtime.

## `@override`

(Originally specified by `698`.)
...
In Python, this type is spelled `Any`. Because `!Any` indicates a
statically unknown type, the static type checker can't check type correctness
of operations on expressions typed as `!Any`. These operations are still
dynamically checked, via the Python runtime's usual dynamic checking.

The Python type system also uses `...` within `Callable` types and
within `tuple[Any, ...]` (see `tuples`) to indicate a statically unknown
component of a type. The detailed rules for these usages are discussed in their
respective sections of the specification. Collectively, along with `Any`,
these are gradual forms.

This specification describes a gradual type system for Python.

## Fully static and gradual types

We will refer to types that do not contain a `gradual form` as a sub-part
as **fully static types**.

A **gradual type** can be a fully static type, `Any` itself, or a type
...
`Any` represents an unknown static type. It denotes some unknown set of
runtime values.
...
An expression typed as `Any`, on the other hand, should be assumed to have
...
if `!Any` might represent a static type which would avoid the error. (This
is defined more precisely below, in terms of materialization and
assignability.)

Similarly, a type such as `tuple[int, Any]` (see `tuples`) or ``int |
Any`` (see `union-types`) does not represent a single set of Python
objects; rather, it represents a (bounded) range of possible sets of values.

In the same way that `Any` does not represent "the set of all Python
...
`Any` allows gradually adding static types to a dynamically typed program.
In a fully dynamically typed program, a static checker assigns the type
`!Any` to all expressions, and should emit no errors. Inferring static
...
Other types (e.g. `Protocols` and `TypedDict`) instead describe a set
...
Since `Any` represents an unknown static type, it does not represent any
...
can also materialize a `Callable` type by replacing `...` with any type
...
`!Any`. (This follows from the definitions of materialization and
...
* [Giuseppe Castagna, Victor Lanvin, Tommaso Petrucciani, and Jeremy G. Siek. 2019. Gradual Typing: A New Perspective.](https://doi.org/10.1145/3290329) Proc. ACM Program. Lang. 3, POPL, Article 16 (January 2019), 112 pages
* [Victor Lanvin. A semantic foundation for gradual set-theoretic types.](https://theses.hal.science/tel-03853222/file/va_Lanvin_Victor.pdf) Computer science. Université Paris Cité, 2021. English. NNT : 2021UNIP7159. tel-03853222
...
behave like stdlib `~dataclasses.dataclass`.
...
* KW_ONLY sentinel values are supported as described in `the Python
 ...
 and [bpo-43532](https://bugs.python.org/issue43532).

* ClassVar attributes are not considered dataclass fields and are
 [ignored by dataclass mechanisms](https://docs.python.org/3/library/dataclasses.html#class-variables).
...
If the two types are `equivalent` but syntactically different,
...
The second argument must be a valid `type expression`.
...
the type of `x` is `t`. `t` must be a valid `type expression`.
...
by using `inspect.cleandoc` or equivalent logic.
...
If a method is marked with the @override decorator
...
For example, an object may implement a `typing.Protocol`, but one
...
described in `stub-file-supported-constructs` below. Type checkers should
...
corresponding "real" module. See `mro` for more information.

### Syntax

Stub files are syntactically valid Python files with a `.pyi` suffix. They
should be parseable (e.g., with `ast.parse`) in all Python versions
...
* All features from the `typing` module of the latest released Python version
 that use supported syntax
* Comments, including type declaration (`# type: X`) and error suppression
 (`# type: ignore`) comments
* Import statements, including the standard `import-conventions` and cyclic
 imports
...
* Any value that is a
 legal parameter for typing.Literal
...
The Literal shortcut using Final should be
supported:
...
The [typeshed project](https://github.com/python/typeshed) contains type
...
the guide to providing type annotations.

For namespace packages (see `420`), the `py.typed` file should be in the
...
For namespace packages (see `420`), stub-only packages should
...
order below, namely `inline` packages and any third-party stubs the type
...
package (see `420`), the `py.typed` file should be in the
...
- Imported symbols are considered private by default. A fixed set of
 import forms re-export imported symbols.
...
Or, since Python 3.12 (`695`), by using the new syntax for
generic functions:
...
* Include a `Protocol` base class parameterized with type variables. This
 ...
 generic protocols for more information.
...
specify type variables for their parameters. See `generic-base-classes`
for details.
...
implicitly) for the type variable must be `assignable` to the bound.
Example:
...
the actual type is `assignable` to the bound.
...
<https://en.wikipedia.org/wiki/Covariance_and_contravariance_%28computer_science%29>`_ and in `483`; here we just show how to control
...
this specification states that when a
...
In accordance with Multiple Type Variable Tuples: Not Allowed, at most one
...
With the new Python 3.12 syntax for generics (introduced by `695`), this can
...
literal "`...`" or another in-scope `ParamSpec` (see Scoping Rules).
...
types or an unpacked, in-scope `TypeVarTuple` (see Scoping Rules).
...
`T1`'s bound must be `assignable` to `T2`'s bound.
...
`assignable` to `bound`. If not, the type checker should generate an
error.
...
have a `LinkedList` whose elements must be `assignable` to the current
class.
...
without giving it an initial value. This can be done using `526`
...
that argument by keyword. Before Python 3.8 (`570`), Python did
...
Consistent with `570` syntax, positional-only parameters cannot appear
...
Before Python 3.9 (`585`), standard library generic types like
...
Before Python 3.10 (`604`), Python did not support the `|` operator
...
`646`, which introduced `TypeVarTuple` into Python 3.11, also made two grammar
...
This also means non-Literal types in general are not `assignable` to
...
**Note:** See Interactions with Final for how we can
...
This document was created following the acceptance of `729`
to serve as a specification for the Python type system. The
initial text consists of the "Specification" sections of `484`
...
- Minor, non-substantive changes can simply be proposed as PRs to
 the [python/typing](https://github.com/python/typing) repository,
...
- Major changes should go through the PEP process, as described in
 `1`. What counts as a major change is not precisely defined,
 but it would generally include any change of a similar magnitude
 to [previous typing PEPs](https://peps.python.org/topic/typing/).

Changes that need Typing Council approval go through three steps:

- Open a discussion on [discuss.python.org](https://discuss.python.org/c/typing/32)
 describing the issue.
- Open a PR on [python/typing](https://github.com/python/typing)
 that changes the spec and, if applicable, the
 [conformance test suite](https://github.com/python/typing/tree/main/conformance).
- [Open an issue](https://github.com/python/typing-council/issues/new) on
 the Typing Council's issue tracker asking for a decision.

The Typing Council has [published](https://github.com/python/typing-council/blob/main/README.md)
...
A named tuple is `assignable` to a `tuple` with a known length and
...
Formally, type *NP* should be narrowed to `A \land R`,
...
with -- its handling of `isinstance`. This guidance allows for changes
...
It is an error to narrow to a type that is not `assignable` to the input
type:
...
signature of the implementation should be `assignable` to the input
...
materializations of the argument's type are assignable to
...
for all remaining overloads are `equivalent`, proceed to step 6.
...
(Originally specified in `544`.)

## Terminology

The term *protocols* is used for some types supporting `structural`
...
Protocols are defined by including a `special form` `typing.Protocol`
...
signature is `assignable` to `SupportsClose.close`, it would implicitly
...
(see "The meaning of annotations"). Bodies of protocol methods are type checked.
...
variables, the special ClassVar annotation should be used.
...
implicit and only `structural` -- the semantics of inheritance is not
changed. Examples:
...
to be one. We still slightly prefer `nominal` subtyping over structural
...
the `SupportsClose` protocol from the example in the protocol-definition section
with ``typing.Sized``:
...
class names can be given as strings. Recursive
...
* A concrete type `X` is assignable to a protocol `P` if and only if `X`
 ...
 words, assignability with respect to a protocol is
 always `structural`.
...
(non-protocol) consistent subtypes of `Proto`.
...
This chapter describes the behavior of some type qualifiers.
...
The generated `__init__` method of `dataclasses` qualifies for this
...
`Final` cannot be used as a qualifier for a TypedDict
item or a NamedTuple field. Such usage also generates
an error at runtime.
 ...
 * If `Annotated` is used in a place where arbitrary
 annotation expressions are allowed,
 the base expression may be an annotation expression.
 * Otherwise, the base expression must be a valid `type expression`.
...
* As with most special forms, `Annotated` is not assignable to
 ``type`` or ``type[T]``:
...
`593` and an earlier version of this specification used the term
...
Every type is `assignable` to `Any`, and `Any` is assignable to every
type.

See `type-system-concepts` for more discussion of `Any`.
...
Since Python 3.11, the `typing` module contains a `special form`
...
Any other special forms like `Callable` are not
...
The type `tuple[Any, ...]` is special in that it is `consistent` with
all tuple types, and `assignable` to a tuple of any length. This is
...
portion of the tuple is `consistent` with any tuple of any length.
...
with `484`. This document is intended to replace those PEPs, and
...
to be interpreted as described in `2119`.

# Typed dictionaries

(Originally specified in `589`, with later additions: `Required`
and `NotRequired` in `655`, use with `Unpack` in `692`,
`ReadOnly` in `705`, and `closed=True` and `extra_items=` in `728`.)
...
A TypedDict is a `structural` type: independent TypedDict types may be
`assignable` to each other based on their structure, even if they do not
share a common base class. For example, two TypedDict types that contain the same
items are `equivalent`. Nevertheless, TypedDict types may inherit from other
...
* `total`: a boolean literal (`True` or `False`) indicating whether
 all items are `required` (`True`, the default) or `non-required`
...
* `closed`: a boolean literal (`True` or `False`) indicating whether
 the TypedDict is `closed` (`True`) or `open` (`False`).
 The latter is the default, except when inheriting from another TypedDict that
 is not open (see `typeddict-inheritance`), or when the `extra_items`
...
* `extra_items`: indicates that the TypedDict has `extra items`. The argument
 must be a `annotation expression` specifying the type of the extra items.
 The `type qualifier` `ReadOnly[]` may be used to indicate that the extra items are
 `read-only`. Other type qualifiers are not allowed. If the extra items type
 is `Never`, no extra items are allowed, so this is equivalent to `closed=True`.

The body of the class definition defines the items of the
...
statically evaluate are also permitted (e.g.
...
To determine whether an item is `required` or `non-required`, the following
procedure is used:
...
variable definition syntax introduced in `526`. On the other hand, this syntax
...
* It can be used in type expressions to
 represent the TypedDict type.

* It can be used as a callable object with keyword arguments
 corresponding to the TypedDict items; see `typeddict-constructor`.
...
Every `required` item must be provided as a keyword argument. `Non-required`
...
As discussed under `typeddict-class-based-syntax`, TypedDict types
...
that is `assignable` to the superclass type. Otherwise, changing the type of an item is not allowed.
Example:
...
The openness of a TypedDict (whether it is `open`, `closed`, or has `extra items`)
...
- If the base class has read-only extra items, the child class may be closed,
 or it may redeclare its extra items with a type that is `assignable` to the base class type.
...
- If `extra_items` is read-only
 ...
 - The item's value type must be `assignable` to `T`

- If `extra_items` is not read-only

 - The item must be non-required
 - The item's value type must be `consistent` with `T`
...
The rest of this section discusses the subtyping rules for TypedDict in more detail.
As with any type, the rules for assignability can be derived from the subtyping
rules using the materialization procedure. Generally, this means that where
"`equivalent`" is mentioned below, the consistency relation can be used instead
when implementing assignability, and where "subtyping" between elements of a
...
- For each item in `A`:
 ...
 (For assignability between two TypedDicts, the first item must instead
 be assignable to the second.)

 - If it is mutable in `A`, it must also be mutable in `B`, and the item type in `B` must be
 `equivalent` to the item type in `A`. (It follows that for assignability, the two item types
 must be `consistent`.)
...
- If `A` has mutable extra items, `B` must also have mutable extra items, and the extra items type in `B`
 must be `equivalent` to the extra items type in `A`. Additionally, for any items in `B` that are not present in `A`,
 the item type must be `equivalent` to the extra items type in `A`.
...
- The TypedDict type has mutable `extra items` of a type that is `equivalent` to `VT`.
- All items on the TypedDict satisfy the following conditions:

 - The value type of the item is `equivalent` to `VT`.
...
Sometimes, operations on `closed` TypedDicts or TypedDicts with
`extra items` are safe even if they would be unsafe on
`open` TypedDicts, so type checker behavior may depend on the
...
Type checkers should allow final names with
...
* As an exception to the general rule around non-literal keys, `d.get(e)` and `e in d`
 ...
 (This simplifies to `object` if `d` is `open`.)

* `clear()` is not safe on `open` TypedDicts since it could remove required items, some of which
 may not be directly visible because of `structural`
 assignability. However, this method is safe on
 `closed` TypedDicts and TypedDicts with `extra items` if
...
* `popitem()` is similarly unsafe on many TypedDicts, even
 if all known items are `non-required`.
...
* Type checkers may allow reading an item using `d['x']` even if
 ...
 with arbitrary str keys when a TypedDict is `closed` or has `extra items`.
 For example:
...
* The return types of the `items()` and `values()` methods can be determined
 ...
 for `open` TypedDicts). Therefore, type checkers should infer more precise
