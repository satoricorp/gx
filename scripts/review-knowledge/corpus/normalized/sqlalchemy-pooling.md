...
which integrate with the `_engine.Engine`. They can also be used
directly for applications that want to add pooling to an otherwise
plain DBAPI approach.

## Connection Pool Configuration

The `_engine.Engine` returned by the
`~sqlalchemy.create_engine` function in most cases has a `.QueuePool`
...
directly to `~sqlalchemy.create_engine` as keyword arguments:
...
fine for `_sa.create_engine` to default to using a `.QueuePool`
...
The usual way to use a different kind of pool with `_sa.create_engine`
...
See the section `custom_dbapi_args` for a rundown of the various
connection customization routines.

## Constructing a Pool

To use a `_pool.Pool` by itself, the `creator` function is
...
`_pool.Pool.connect` function. The return value of this method is a DBAPI
...
The "reset on return" feature takes place when a connection is `released`
back to the connection pool. In modern SQLAlchemy, this reset on return
behavior is shared between the `.Connection` and the `.Pool`,
where the `.Connection` itself, if it releases its transaction upon close,
...
autocommit or when using a database
...
As of SQLAlchemy 2.0.43, the `.create_engine.skip_autocommit_rollback`
parameter of `.create_engine` provides the most complete means of
...
Detail on this pattern is at `dbapi_autocommit_skip_rollback`.

The `_pool.Pool` itself also has a parameter that can control its
...
`_pool.Pool.reset_on_return` parameter of `_pool.Pool`, which
is also available from `_sa.create_engine` as
`_sa.create_engine.pool_reset_on_return`, passing a value of `None`.
...
The above pattern will still see ROLLBACKs occur however as the `.Connection`
...
`.PoolEvents.reset` event hook. The
`_sa.create_engine.pool_reset_on_return` parameter is set to `None`
...
`_sa.create_engine.echo_pool` to `"debug"` when using
...
checkin of connections. See `_events.PoolEvents` for details.
...
using the `_pool.Pool.pre_ping` argument, available from `_sa.create_engine`
...
`_engine.Connection` object will detect a "disconnect" situation and
...
`dbapi_autocommit`, a connection **may** be reconnected transparently
mid-operation using events. See the section `faq_execute_retry` for
an example.
...
error messages using the `_events.DialectEvents.handle_error` hook.

#### Custom / Legacy Pessimistic Ping

Before `_sa.create_engine.pool_pre_ping` was added, the "pre-ping"
approach historically has been performed manually using
the `_events.ConnectionEvents.engine_connect` engine event.
...
a "disconnect" situation, as well as the `_engine.Engine` object's ability
to correctly invalidate the current connection pool when this condition
occurs and allowing the current `_engine.Connection` to re-validate onto
...
`_pool.Pool` is used in conjunction with a `_engine.Engine`.
The `_engine.Engine` has logic which can detect
disconnection events and refresh the pool automatically.

When the `_engine.Connection` attempts to use a DBAPI connection, and an
exception is raised that corresponds to a "disconnect" event, the connection
is invalidated. The `_engine.Connection` then calls the `_pool.Pool.recreate`
...
of the `_pool.Pool` itself, independent of whether or not an `_engine.Engine` is in use.

### More on Invalidation

The `_pool.Pool` provides "connection invalidation" services which allow
...
When using a `_engine.Engine`, the `_engine.Connection.invalidate` method is
the usual entrypoint to explicit invalidation. Other conditions by which
a DBAPI connection might be invalidated include:

* a DBAPI exception such as `.OperationalError`, raised when a
...
* When a listener implementing `_events.PoolEvents.checkout` raises the
...
All invalidations which occur will invoke the `_events.PoolEvents.invalidate`
event.
...
`_events.DialectEvents.handle_error` event hook, which is typically
established via the owning `_engine.Engine` object. Using this hook, all
errors which occur are delivered passing along a contextual object known as
`.ExceptionContext`. Custom event hooks may control whether or not a
...
The `.QueuePool` class features a flag called
`.QueuePool.use_lifo`, which can also be accessed from
`_sa.create_engine` via the flag `_sa.create_engine.pool_use_lifo`.
...
Above, we also make use of the `_sa.create_engine.pool_pre_ping` flag
...
using an `_engine.Engine` created via `_sa.create_engine`, that
...
The SQLAlchemy `_engine.Engine` object refers to a connection pool of existing
...
1. Disable pooling using `.NullPool`. This is the most simplistic,
 one shot system that prevents the `_engine.Engine` from using any connection
 more than once:
...
2. Call `_engine.Engine.dispose` on any given `_engine.Engine`,
 passing the `.Engine.dispose.close` parameter with a value of
...
process.
3. Call `.Engine.dispose` **directly before** the child process is
...
```
 ...
 `pool_disconnects_pessimistic` to treat a DBAPI connection that
...
The above strategies will accommodate the case of an `_engine.Engine`
being shared among processes. The above steps alone are not sufficient for the
case of sharing a specific `_engine.Connection` over a process boundary;
prefer to keep the scope of a particular `_engine.Connection` local to a
...
ORM `_orm.Session` object that's begun a transaction and references
active `_orm.Connection` instances; again prefer to create new
`_orm.Session` objects in new processes.
