...
The `dataclasses` module allows defining and customizing simple
...
Some functions in the `dataclasses` module, such as `~dataclasses.asdict`,
...
as a dataclass decorator, consider using the `~typing.dataclass_transform`
...
Mypy supports the `~typing.dataclass_transform` decorator as described in
[PEP 681](https://www.python.org/dev/peps/pep-0681/#the-dataclass-transform-decorator).
...
* Validator decorators
 and [default decorators](https://www.attrs.org/en/stable/examples.html#defaults)
...
As an experimental mypy extension, you can specify `~collections.abc.Callable` types
that support keyword arguments, optional arguments, and more. When
you specify the arguments of a `~collections.abc.Callable`, you can choose to supply just
...
corresponding `~collections.abc.Callable`:
...
`~collections.abc.Callable` such as
...
A `~collections.abc.Callable` with unspecified argument types, such as
...
See `dynamic-typing` for more details.
...
See `generic-builtins` for more details.
...
See `type-inference-and-annotations` for more details.
...
`runtime_troubles` for dealing with this.
...
You can even make your own duck types using `protocol-types`.
...
See `forward-references` for more details.

## Decorators

Decorator functions can be expressed via generics. See
`declaring-decorators` for more details. Example using Python 3.12
syntax:
...
See `async-and-await` for the full detail on typing coroutines and asynchronous code.
...
can override this with a `~typing.ClassVar` annotation.)
...
`~typing.ClassVar`. An attribute without the `~typing.ClassVar` annotation can
...
Note that `~typing.ClassVar` is not a class, and you can't use it with
...
a `~typing.ClassVar` annotation, but this might not do what you'd expect:
...
An explicit `~typing.ClassVar` may be particularly handy to distinguish
...
even if you omit the `~abc.ABCMeta` metaclass. This can be useful if the
...
for full details, see `running-mypy`.
...
The `Any` type is used to represent a value that has a dynamic type.
...
other are type-checked in parallel. Incremental cache is
used to manage most of the shared state. Parallel type-checking also requires
--local-partial-types, which is
...
* Parallel mode requires and automatically enables `--native-parser`.
...
* `PreciseTupleTypes`: this feature will infer more precise tuple types in
 ...
 by `646`, it was impossible to express a type like "a tuple with
...
reveal_type() might come in handy.
...
* To replace the contents of a module with `Any`, use a per-module `follow_imports = skip`.
 See Following imports for details.
...
For dealing with these, see `runtime_troubles`.
...
a factor of 10 or more. Remote caching can
make cold mypy runs several times faster.

Furthermore: as of [mypy 1.13](https://mypy-lang.blogspot.com/2024/10/mypy-113-released.html),
...
by default (see `variance-of-generics` for motivation). When using the
`695` syntax (`class MyClass[T]: ...`), variance is inferred from
...
`~typing.TYPE_CHECKING`, variables named `MYPY` or `TYPE_CHECKING`, and any variable
...
in --python-version 3.8 from the command line. Note that you do not need
...
[mypy repository on GitHub](https://github.com/python/mypy), and then run
`pip install` locally:
...
instructions at the [mypyc wheels repo](https://github.com/mypyc/mypy_mypyc-wheels).
...
supported Python version and platform checks):
...
If you use the --warn-unreachable flag, mypy will generate
...
an existing codebase. See `existing-code` for concrete advice for
that situation.
...
The --config-file command-line flag has the
...
For more information, see the Import discovery
...
For more information, see the Disallow dynamic typing
section of the command line docs.
...
For more information, see the Untyped definitions and calls
section of the command line docs.
...
For more information, see the None and Optional handling
section of the command line docs.
...
For more information, see the Configuring warnings
section of the command line docs.
...
For more information, see the Miscellaneous strictness flags
section of the command line docs.
...
For more information, see the Configuring error messages
section of the command line docs.
...
what is allowed in a `toml` file. See PEP 518 for more information on the layout
...
You can also often use `protocol-types` to achieve a similar effect in
...
In `getting-started-dynamic-vs-static`, we discussed how bodies of functions
...
with default options. See `error-codes` for general documentation
about error codes. `error-codes-optional` documents additional
...
You can use `~collections.abc.Callable` as the type for callable objects:
...
[Python documentation](https://docs.python.org/3.13/reference/datamodel.html#determining-the-appropriate-metaclass)
...
See `ignore-missing-imports` for how to work around these errors.
...
but that module does not provide type annotations (via PEP 561).
...
stub package. See `ignore-missing-imports` for more details.
...
abstract method or attribute. (See also `abc` module documentation)
...
The target of a `~typing.NewType` definition must be a class type. It can't
...
for `acme` or create a stub file for the module. See `ignore-missing-imports`
for more information.
...
--check-untyped-defs or
--disallow-untyped-defs). To avoid
...
if you enable certain options. See `error-codes` for general
documentation about error codes and their configuration.
`error-code-list` documents error codes that are enabled by default.
...
explicitly. Such annotations are allowed by `673` but are
...
--strict-equality does not include comparisons with
`None`:
...
--strict-equality-for-none (we might merge
...
If you use --warn-unreachable, mypy generates an error if it
...
If you use --enable-error-code deprecated,
...
specified in [PEP 702](https://peps.python.org/pep-0702).
Use the --report-deprecated-as-note option to
turn all such errors into notes.
Use --deprecated-calls-exclude to hide warnings
...
or `__new__`. See [PEP 698](https://peps.python.org/pep-0698/#strict-enforcement-per-project).
...
But, in Python3.11 `typing.reveal_type` was added.
...
If you use --disallow-any-explicit, mypy generates an error
...
If enabled with --enable-error-code exhaustive-match,
...
If enabled with --disallow-untyped-decorators
...
2. The error code can be used to find documentation about the error.
 The next two topics (`error-code-list` and
 `error-codes-optional`) document the various error codes
 mypy can report.
...
certain optional error codes, such as --disallow-untyped-defs,
...
You can use configuration file sections to enable or
...
See ignore-without-code for more information.
...
* Make sure everyone runs mypy with the same options. Checking a mypy
 configuration file into your codebase is the
 easiest way to do this.

* Make sure everyone type checks the same set of files. See
 `specifying-code-to-be-checked` for details.
...
type check. The `ignore_errors` option is useful for this, for instance,
...
various error codes on a per-module basis, see `error-codes`.
...
See `fix-missing-imports` for a complete reference on these errors
...
case, we recommend using a configuration file. For example,
...
or setting `ignore_missing_imports` to true globally.
...
`monkeytype:index`, autotyping and PyAnnotate.
...
`disallow_untyped_defs`. Refer to `config-file` for the details.
...
`command-line` reference and `error-codes-optional`.

## Speed up mypy runs

You can use mypy daemon to get much faster
...
so, you may also want to set up remote caching
...
static types. Extending the `484` type system to accommodate all existing
...
that uses a library that is difficult to express using just `484` types.
...
config file using the `plugins` option and one of the two formats: relative or
...
full name. See the full plugin hook method list below.
...
For example, `484` doesn't support defining variadic generic types:
...
This includes special Python methods except `~object.__init__` and `~object.__new__`.
...
Unlike above, this does not have special casing for __getattr__ or
__getattribute__.
...
See also the [front page](https://www.mypy-lang.org) of the mypy web
site.
...
such as `~collections.abc.Iterable` that always support structural
...
subtyping see `protocol-types` and `544`.
...
checking. The section `common_issues` mentions some
...
Cython is a variant of Python that supports
...
* Finally, you can write `self.id: Final = 1` (also optionally with
 ...
 __init__ methods so the final instance attribute is
...
`Final` and `~typing.ClassVar` should not be used together. Mypy will infer
...
[new dedicated syntax](https://docs.python.org/3/whatsnew/3.12.html#pep-695-type-parameter-syntax)
...
When using the legacy syntax, Generic can be omitted
...
simpler syntax, introduced in `673`, to make them easier to use.
...
* Most immutable container types, such as `~collections.abc.Sequence`
 and `~frozenset` are covariant. Union types are
...
* `~collections.abc.Callable` is an example of type that behaves contravariant
...
* `list` is an invariant generic type. Naively, one would think
 that it is covariant, like `~collections.abc.Sequence` above, but consider this code:
...
where the return type would be `S` (see `type-variable-upper-bound`).
...
Pattern[S] is used for the return
value of `re.compile`, where `S` can be either `str`
...
Note that you may come across `~typing.AnyStr` imported from
`typing`. This feature is now deprecated, but it means the same
...
`~typing.cast` to convince mypy that `wrapper()` has the same
signature as `func` (see casts).
...
problem. This is also the reason for the `~typing.cast` call in the
...
The same is possible using the legacy syntax with `~typing.ParamSpec`
...
achieved by combining with @overload (Python 3.12 syntax):
...
Mypy supports generic protocols (see also `protocol-types`). Several
predefined protocols are generic, such as
Iterable[T], and you can define additional
...
as per PEP 544: Generic protocols.
...
See `variance-of-generics` for more about variance.
...
PEP 484: Type aliases, Python 3.11 and earlier):
...
`list`, `dict` and others do not support indexing.
This is why we have the aliases `~typing.List`,
`~typing.Dict` and so on in the `typing`
...
functions by using the --check-untyped-defs flag.
See `command-line` for more information on configuring mypy.
...
If you run mypy with the --strict flag, you
...
to add static types to a large, existing codebase. See `existing-code`
...
--ignore-missing-imports
may be useful. See `getting-to-strict` for how to build up to `--strict`.

See `command-line` and `config-file` for a complete reference on
...
For more details, see `type-inference-and-annotations`.
...
`Path` object from the pathlib standard library module:
...
If a third party library you use declares support for type checking,
...
type information, refer to `fix-missing-imports`.
...
* Read `existing-code` if you have a significant existing
...
* Look at solutions to common issues with mypy if
...
correctly. With mypy, add type hints (`484`)
...
Flags correspond to config file flags but allow
...
`types-<library>`. (See `fix-missing-imports` for how to deal
...
--python-executable flag to point
...
package.
`561` describes three main ways to distribute type
information:

1. A package has inline type annotations in the Python implementation.

2. A package ships stub files with type
...
`object` type, which represents the set of all values.
Unlike `object`, `Any` introduces type unsafety — see
`any-vs-object` for more.
...
The `Any` type is discussed in more detail in section `dynamic-typing`.
...
types. You can use overloading to
...
`from __future__ import annotations` (see `runtime_troubles`):
...
You can also use the `~typing.Optional` type modifier, such as
...
There is also use an older syntax for defining explicit type aliases (`613`):
...
`type-variable-upper-bound`) we can do better (Python 3.12 syntax):
...
For more details about `type[]` and typing.Type[], see :pep:`PEP 484: The type of
...
Iterator[YieldType] over
Iterable[YieldType] as the return-type annotation for a
generator function, as it lets mypy know that users are able to call `next` on
...
If you want your generator to accept values via the `~generator.send` method or return
a value, on the other hand, you should use the
Generator[YieldType, SendType, ReturnType] generic type instead of
...
`~typing.Generator` behaves contravariantly, not covariantly or invariantly.
...
since generators have `~generator.close`, `~generator.send`, and `~generator.throw` methods that
generic iterators and iterables don't. If you plan to call these methods on the returned
generator, use the `~typing.Generator` type instead of `~typing.Iterator` or `~typing.Iterable`.
...
you can instead change the variable to be `Final` (see `final_attrs`):
...
Mypy has special support for `enum.Enum` and its subclasses:
`enum.IntEnum`, `enum.Flag`, `enum.IntFlag`,
and `enum.StrEnum`.
...
- Any `Enum` class with values is implicitly final.
...
`~enum.Enum`\s, `~typing.NamedTuple`\s and singletons.

Mypy has some special understanding of `~abc.ABCMeta` and `EnumMeta`.
...
For some builtin types, mypy may think their metaclass is `abc.ABCMeta`
even if it is `type` at runtime. In those cases, you can either:

* use `abc.ABCMeta` instead of `type` as the
...
This section introduces a few additional kinds of types, including `~typing.NoReturn`,
`~typing.NewType`, and types for async code. It also discusses
...
* `~typing.NoReturn` lets you tell mypy that a function never returns normally.

* `~typing.NewType` lets you define a variant of a type that is treated as a
...
Mypy will ensure that functions annotated as returning `~typing.NoReturn`
...
pip to use `~typing.NoReturn` in your code. Python 3 command line:
...
module provides a helper object `~typing.NewType` that creates simple unique types with
...
`~typing.NewType` accepts exactly two arguments. The first argument must be a string literal
...
The callable returned by `~typing.NewType` accepts only one argument; this is equivalent to
...
`~typing.NewType`, nor can you subclass an object returned by `~typing.NewType`.
...
`slice`, we return a `~collections.abc.Sequence` of items.
...
In particular, an `~object.__init__` method overloaded on self-type
may be useful to annotate generic class constructors where type arguments
depend on constructor parameters in a non-trivial way, see e.g. `~subprocess.Popen`.
...
For more information regarding coroutines, see `492` and the
[asyncio documentation](python:library/asyncio).
...
Coroutine[Any, Any, T], which is a subtype of
Awaitable[T]:
...
`~collections.abc.AsyncIterator` type in your annotations:
...
Async generators (introduced in `525`) are an easy way to create
async iterators:
...
changed a few files. (You can use remote caching
...
IDEs, and other tools (for example, the mypy plugin for PyCharm),
...
Important note: it is recommended to check files with `--export-types`
since otherwise most inspections will not work without `--force-reload`.
...
native isinstance check works -- based on class
hierarchy.
...
well known to Python programmers. See `544` for the detailed specification
of protocols and structural subtyping in Python.

## Predefined protocols

The `collections.abc`, `typing` and other stdlib modules define
various protocol classes that correspond to common Python protocols, such as
Iterable[T]. If a class
...
`predefined_protocols_reference` lists various protocols defined in
`collections.abc` and `typing` and the signatures of the corresponding methods
...
`isinstance` also works with the predefined protocols
in `typing` such as `~typing.Iterable`.
...
Callback protocols and `~collections.abc.Callable` types can be used mostly interchangeably.
Parameter names in __call__ methods must be identical, unless
...
See also `~collections.abc.Container`.
...
See also `~collections.abc.Collection`.
...
See also `~collections.abc.Reversible`.
...
See also `~typing.SupportsComplex`.
...
See also `~collections.abc.Awaitable`.
...
See also `~collections.abc.AsyncIterable`.
...
See also `~collections.abc.AsyncIterator`.
...
See also `~contextlib.AbstractContextManager`.
...
See also `~contextlib.AbstractAsyncContextManager`.

# Running mypy and managing imports

The `getting-started` page should have already introduced you
...
```
 ...
 For more details, see `finding-imports`.
...
You can also use the `files` option in your `mypy.ini` file to specify which
...
--no-namespace-packages and
--explicit-package-bases are set.

1. If --no-namespace-packages is set,
...
3. You'll notice that the above case still relies on `__init__.py`. If
 ...
 flags), --explicit-package-bases
 provides a solution.

 With --explicit-package-bases, mypy
 will locate the nearest parent directory that is a member of the `MYPYPATH`
 environment variable, the `mypy_path` config or is the current
...
Passing -v will show you the files and associated module
...
`dynamic-typing` for more information.
...
PEP 561 compliant stub package (e.g. with a `py.typed` file) or have registered
themselves on [typeshed](https://github.com/python/typeshed), the repository
...
2. Searching to see if there is a PEP 561 compliant stub package
 ...
 install the [django-stubs](https://pypi.org/project/django-stubs/) package.

3. Writing your own stub files containing type hints for
...
4. Force mypy to analyze the library as best as it can (as if the library provided
 ...
 command line flag or `follow_untyped_imports` config file option to True.
...
2. To suppress *all* missing import errors from a single library, add
 a per-module section to your mypy config file setting
 `ignore_missing_imports` to True for that library. For example,
...
.. code-block:: toml
 ...
 import discovery in config files.
...
3. To suppress *all* missing import errors for *all* untyped libraries
 in your codebase, use --disable-error-code=import-untyped.
 See `code-import-untyped` for more details on this error code.
...
.. code-block:: ini
 ...
 [tool.mypy]
 disable_error_code = ["import-untyped"]
 You can also set the --ignore-missing-imports
 command line flag or set the `ignore_missing_imports` config file
...
Alternatively, add the --install-types
...
By default, --install-types shows a confirmation prompt.
Use --non-interactive to install all suggested
...
3. Reading the `finding-imports` section below to make sure you
...
4. Directly specifying the directory containing the module you want to
 type check from the command line, by using the `mypy_path`
 or `files` config file options,
...
- The directories containing the sources given on the command line
 (see Mapping file paths to modules).
- The installed packages marked as safe for type checking (see
 PEP 561 support)
- The relevant directories of the
 [typeshed](https://github.com/python/typeshed) repo.
...
Mypy is designed to doggedly follow all imports,
...
by using the --follow-imports flag. This flag
...
you want to type check and using --follow-imports=silent.
...
a given object has). See `existing-code` for more recommendations.
...
* Use of `from __future__ import annotations` (`563`)
...
with import cycles, see `import-cycles`.)
...
required to be valid Python syntax. For more details, see `563`.
...
The `typing` module defines a `~typing.TYPE_CHECKING` constant
...
runtime. This is most useful for resolving import cycles.
...
future annotations import, or b) string literals or type
...
Some classes are declared as generic in stubs, but not
at runtime.

In Python 3.8 and earlier, there are several examples within the standard library,
for instance, `os.PathLike` and `queue.Queue`. Subscripting
...
future annotations import (or string literals or type
...
since its `queue.Queue` implements `~object.__class_getitem__`, so
...
Starting with Python 3.9 (`585`), the type objects of many collections in
the standard library support subscription at runtime. This means that you no
longer have to import the equivalents from `typing`; you can simply use
the built-in collections or those from `collections.abc`:
...
notes at future annotations import.

## Using X | Y syntax for Unions

Starting with Python 3.10 (`604`), you can spell union types as
...
the notes at future annotations import.

## Using new additions to the typing module

You may find yourself wanting to use features added to the `typing`
...
This plays nicely well with following `508` dependency specification:
...
A stub file (see `484`) contains only type hints for the public
...
[typeshed](https://github.com/python/typeshed) repository to determine
...
that `561` stub-only packages must be installed, and may not be pointed
at through the `MYPYPATH` (see PEP 561 support).
...
example, by using the capabilities of the `inspect` module. Stubtest
...
Run `stubtest --help` for a quick summary of options.
...
more in `variance`. For example, the following program generates a mypy error,
...
typing it with `Any`. See `dynamic-typing` for more information.
...
You can also specify per-module configuration options in your `config-file`.
For example:
...
don't perform a runtime type check. Use the function `~typing.cast`
to perform a cast:
...
Mypy supports User-Defined Type Guards (`647`).
...
Mypy supports TypeIs (`742`).

A [TypeIs narrowing function](https://typing.readthedocs.io/en/latest/spec/narrowing.html#typeis)
allows you to define custom type checks that can narrow the type of a variable
in [both the if and else](https://docs.python.org/3.13/library/typing.html#typing.TypeIs)
...
from [typing_extensions](https://typing-extensions.readthedocs.io/en/latest/)
...
it's easy to deduce that if `a` is None then `b` must not be,
therefore `a or b` will always be an instance of `C`,
...
You can use an `assert` to convince the type checker, override it with a cast
...
`~collections.abc.Mapping` only provides read-only access to the dictionary items:
...
You may need to use `~dict.get` to access items of a partial (non-total)
...
Requiring `~dict.get` everywhere would be too cumbersome. (Note that you
are free to use `~dict.get` with total `TypedDict`\s as well.)
...
* Anything included in `~collections.abc.Mapping`:
...
* d1.update(d2)
* d.pop(key[, default]) (partial `TypedDict`\s only)
...
`typing_extensions.ReadOnly` to mark TypedDict items as read-only (`705`):
...
Instead, you can use the tagged union pattern. The referenced
...
Literal type. Then, check that key to distinguish
