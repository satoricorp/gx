...
file like `foo.sh` and a build rule named `foo`.`PATH`, then prefer
...
output of a build rule rather than deploying the source file directly.Libraries must have a `.sh` extension and should not be executable.
...
```
`;;`.`;;` on separate lines.The matching expressions are indented one level from the `case` and `esac`.
...
`$?`, `$#`, `$$`, `$!`. Prefer quoting of
“named” internal integer variables, e.g. PPID etc for consistency.`[[ … ]]`. See the
Test, `[ … ]`, and `[[ … ]]` section below.`"$@"` unless you have a specific reason to use `$*`, such as simply
...
var="`command \`command1\``"
```
`[ … ]`, and `[[ … ]]``[[ … ]]` is preferred over `[ … ]`, `test` and `/usr/bin/[`.
