...
This file defines the domain language lgtm reviews and agent workflows should use.
...
The product layer over JJ and Git. lgtm owns the workflow for capturing coding
context, shaping reviewable revisions, publishing stacks, and preparing review
evidence.

lgtm is not a generic Git wrapper, a JJ teaching tool, or a generic VCS
abstraction.
...
One reviewable logical change. A Revision is the lgtm product identity for a
change.
...
An ordered line of Revisions. Locally, a Stack is lgtm metadata plus a JJ/Git
compatible ref. Remotely, it can be published as stacked Git branches and PRs.

### Published Stack

A Stack that lgtm has exported to the remote Git or review surface.

### Compose

The normal lgtm authoring flow that turns messy working-copy changes into
...
The Go module that owns lgtm authoring behavior such as compose, edit, status,
...
- Review output should use lgtm product terms before implementation terms.
