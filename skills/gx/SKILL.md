...
description: >-
  ...
  publish with `git push` (never `gx push`), then `gh pr create`. gx adds AI issues and
  tips via `gx enhance`; use it to improve the change, not to save it.
---
...
`gx enhance` reports issues and tips to improve the change you are working on. Run it
from this skill with `GX_CLIENT=skill` set — it labels the run as a skill invocation so
usage is countable per surface:

```bash
GX_CLIENT=skill gx enhance                                    # patch-focused default
GX_CLIENT=skill gx enhance "did we break the retry contract?" # steer with a prompt
GX_CLIENT=skill gx enhance --repo                             # assess the codebase, not just the current change
GX_CLIENT=skill gx enhance --scope security                   # architecture, security, performance, etc.
GX_CLIENT=skill gx enhance --focus src/auth --deep            # deep pass limited to a path prefix
GX_CLIENT=skill gx enhance --verbose                          # include repo facts, docs, changed files
```

Without `--repo`, a dirty working tree is the subject: gx assesses the diff. Use
`--repo` to assess the repository itself — the reviewer is given an inventory of the
repository and its source files, in any language, and findings are no longer filtered
down to changed lines. The uncommitted work stays in focus, so the run still sees
