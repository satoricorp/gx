...
Installing the Totality CLI package provides `tl` and `tl-mcp`. It does not rewrite every repository immediately; each repo gets the
Git hook when it is initialized with `tl init`, initialized automatically by
the CLI or MCP.
...
```bash
tl capture push --remote "$remote" --ref-range "$range" --repo "$(git rev-parse --show-toplevel)"
```
...
## Machine-wide install (`tl init --global`)

`tl init --global` writes the same lifecycle scripts to `~/.totality/hooks`
(`$TOTALITY_HOME/hooks` when set) and points `git config --global core.hooksPath` at
that directory, so Totality works in every repo without a per-repo `tl init`. It does
...
carrying the `# tl lifecycle hooks` marker are skipped so a repo that also ran
plain `tl init` does not run Totality twice.
...
- If `core.hooksPath` is already set globally to a non-Totality directory, install
  ...
  idempotent.
- `git config tl.enabled false` inside a repo excludes it (checked by the Go
...
- `tl doctor` reports the global state and flags an overridden or incomplete
  install; it never repairs it silently.

## Manual run

```bash
tl capture push --ref-range main..HEAD --repo .
```
...
`tl commit`, so a missing post hook cannot lose the revision identity.
