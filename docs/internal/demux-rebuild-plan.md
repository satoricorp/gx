...
and executable specification of lgtm's demux/generate implementation before its
jj-dependent code is removed.

Use it to rebuild the feature only if lgtm again needs to turn a mixed working
...
```text
Repository: satoricorp/lgtm
Commit: 1abd91d
User command at that commit: lgtm generate
...
11. Record planning and provenance evidence against each resulting lgtm Revision.
...
`RevisionID` is the durable lgtm-owned ID described by the Git-native Revision
...
- Generate a durable lgtm Revision ID for each commit.
- Add the lgtm trailer before each Git commit is created.
...
- Squash/rebase after apply preserves lgtm Revision identity and evidence.
...
8. Implement transactional Git apply with lgtm Revision trailers.
...
- each resulting commit has a durable lgtm Revision ID;
