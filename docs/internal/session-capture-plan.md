...
into a SQLite database. When you `git push`, lgtm's `pre-push` hook runs `lgtm
...
is what produces coverage, authorship, and the PR summary. Nothing depends on lgtm
...
and `-Users-joe-git-yeet` that were pulled in while capturing for `lgtm`. Matching
...
3. **Origin → connected repo** matches the normalized origin against the repos
   connected to the user's lgtm org. This is the gate.
...
`lgtm` and `console` — genuinely benefit from each other's context. The boundary is
...
> lgtm indexes agent sessions against the repositories your organization has
