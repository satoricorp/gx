# Totality Authoring Glossary

Totality keeps several names for the same reviewable revision because each layer has its
...
- **Stack**: the named line of revisions. Locally it is backed by Totality metadata
...
- **Exact provenance**: `change_sessions(change_id, session_id)` created from an
  explicit handoff such as `TOTALITY_SESSION_ID` / `TOTALITY_SESSION_IDS`, or from an
...
- **Feasibility warning**: a structured warning attached to a demux proposal
  when Totality can see that a proposed revision may not be reviewable bottom-up.
