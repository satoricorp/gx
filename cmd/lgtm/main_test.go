...
func TestRootKeepsReportAsHiddenAlias(t *testing.T) {
	...
	if !cmd.Hidden {
		t.Fatal("lgtm report should be hidden after folding into lgtm doctor --report")
	}
}
...
func TestRootPrintsInitNoteWhenIdentityMissing(t *testing.T) {
	...
	text := out.String()
	if !bytes.Contains(out.Bytes(), []byte("Run `lgtm init` first.")) {
...
func TestRootSkipsInitNoteWhenIdentityConfigured(t *testing.T) {
	...
	if err := lgtmconfig.Save(lgtmconfig.Config{
		User: lgtmconfig.User{Name: "Joe Example", Email: "joe@example.com"},
	}); err != nil {
